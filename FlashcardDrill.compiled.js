...
//   incorrect/incorrectBg = "Incorrect" difficulty tier (Practice by Difficulty)
...
export default function FlashcardDrillApp() {
    ...
    // continueAfterSolution: the "Continue"/"Next"/"Next Cycle"/"Finish Session" button
    // shown in place of the options once a card with a #solution has been answered
    // correctly (see showSolutionReveal below).
    function continueAfterSolution() {
        setShowSolutionReveal(false);
        handleAdvance();
    }
    // nextButtonLabel: what continueAfterSolution's button should say, computed from
    // exactly the same three-way branch handleAdvance() uses, WITHOUT changing state —
    // "Next" (more cards left in this cycle), "Next Cycle" (this was the cycle's last
    // card but more cycles remain), or "Finish Session" (this was the last card of the
    // last cycle — whether that leads straight to the all-clear summary or, if cards are
    // still flagged, to a review round, is decided by handleAdvance when actually clicked).
    ...
    //   - Correct AND the card has a #solution -> instead of auto-advancing, the
    //     options are replaced with the solution text + a "Continue" button
    //     (showSolutionReveal / continueAfterSolution above). Applies to every
    //     practice mode, not just Review Due — it's independent of sessionAffectsProgress.
    ...
    function handleOptionClick(idx) {
        ...
        if (opt.correct) {
            ...
            setTimeout(() => {
                ...
                // If this card has a #solution, pause here on a "Continue" screen instead
                // of auto-advancing — continueAfterSolution() calls handleAdvance() once
                // the person has read it and tapped through. Cards with no solution keep
                // the original behavior of advancing immediately. Applies to a first-try
                // correct answer just as much as an incorrect-then-correct retry.
    ...
    if (!loaded) {
        ...
        // The Strong / Good / Weak / Incorrect picker. Tile colors:
        // COLORS.green/greenBg, COLORS.mustard/mustardBg, COLORS.red/redBg,
        // COLORS.incorrect/incorrectBg (in that order below).
        // =====================================================================
    }
        ...
        const tiers = [
            ...
            { key: 'incorrect', label: 'Incorrect Cards', color: COLORS.incorrect, bg: COLORS.incorrectBg },
        ];
        content = (React.createElement("div", { className: "flex flex-col gap-3" },
        ...
        if (phase === 'question' && currentCard) {
            ...
            content = (React.createElement(SessionSummaryScreen, { colors: COLORS, cards: cards, sessionPoolIds: sessionPoolIds, sessionResults: sessionResults, onDone: exitPractice }));
        }
    }
...
function SessionSummaryScreen({ colors: COLORS, cards, sessionPoolIds, sessionResults, onDone }) {
    const tiers = [
        { key: 'strong', label: 'Strong Cards', color: COLORS.green, bg: COLORS.greenBg },
        { key: 'good', label: 'Good Cards', color: COLORS.mustard, bg: COLORS.mustardBg },
        { key: 'weak', label: 'Weak Cards', color: COLORS.red, bg: COLORS.redBg },
    ...
    const items = sessionPoolIds.map((id) => {
        const card = cards.find((c) => c.id === id);
    ...
    return (React.createElement("div", { className: "flex flex-col gap-4" },
        React.createElement("div", { className: "flex flex-col items-center gap-1 pt-2" },
            React.createElement("div", { className: "stamp-el rounded-lg px-6 py-4 text-center", style: { border: `4px double ${COLORS.green}`, backgroundColor: COLORS.greenBg, transform: 'rotate(-4deg)' } },
                ...
                React.createElement("div", { style: { fontFamily: "'Space Mono', monospace", color: COLORS.green }, className: "text-xs mt-1" }, `${items.length} card${items.length !== 1 ? 's' : ''} \u00B7 next reviews rescheduled`))),
        React.createElement("div", { className: "grid grid-cols-2 gap-2" }, tiers.map((t) => (React.createElement("div", { key: t.key, className: "rounded-xl border-2 px-3 py-2 flex items-center justify-between", style: { borderColor: t.color, backgroundColor: t.bg } },
        ...
        React.createElement("div", { style: { fontFamily: "'Space Mono', monospace", color: COLORS.inkFaint }, className: "text-xs uppercase tracking-widest" }, "Time per card"),
        React.createElement("div", { className: "flex flex-col gap-2 max-h-72 overflow-y-auto" }, items.map((it, i) => {
            ...
            return (React.createElement("div", { key: it.id, className: "rounded-xl border-2 px-4 py-3", style: { borderColor: COLORS.rule, backgroundColor: COLORS.card } },
                ...
                React.createElement("span", { style: { fontFamily: "'Roboto Slab', serif", color: tier.color, backgroundColor: tier.bg }, className: "inline-block text-[10px] font-bold px-2 py-0.5 rounded-full mt-1" }, tier.label)));
        })),
