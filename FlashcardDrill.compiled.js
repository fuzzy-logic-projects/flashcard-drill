...
export default function FlashcardDrillApp() {
    ...
    // pasted Markdown is never auto-split, per how this feature was scoped.
    // Starts at level 1, due immediately, like a brand new card.
    function importNote(deckId, title, body) {
    ...
    if (!loaded) {
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-3" },
            ...
            React.createElement("div", { className: "flex gap-2" },
                React.createElement("select", { value: noteSort, onChange: (e) => setNoteSort(e.target.value), style: { ...monoStyle, borderColor: COLORS.rule, backgroundColor: COLORS.card, color: COLORS.ink }, className: "flex-1 rounded-lg border-2 px-2 py-2 text-xs focus:outline-none focus:ring-2" },
                ...
                React.createElement("button", { onClick: () => { setNoteImportTitle(''); setNoteImportBody(''); setView('noteImport'); }, style: { backgroundColor: COLORS.accent, ...fontStyle }, className: "rounded-lg px-4 py-2 text-white text-xs font-bold flex items-center gap-1" },
            ...
            React.createElement("div", { className: "flex flex-col gap-2" },
                sortedNotes.map((n) => {
                    ...
                    return (React.createElement("button", { key: n.id, onClick: () => { setSelectedNoteId(n.id); setView('noteRead'); }, className: "w-full text-left rounded-xl border-2 px-4 py-3 flex items-center justify-between gap-2", style: { borderColor: COLORS.rule, backgroundColor: COLORS.card } },
                        React.createElement("span", { style: { ...fontStyle, color: COLORS.ink }, className: "text-sm font-bold" }, n.title),
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-3" },
            React.createElement(BackHeader, { colors: COLORS, onBack: () => setView('notes'), title: "Import Note" }),
            ...
            React.createElement("textarea", { value: noteImportBody, onChange: (e) => setNoteImportBody(e.target.value), rows: 14, placeholder: "Paste your Markdown notes here. Use [ ] for math, e.g. [x^2 + 1]", style: { ...monoStyle, borderColor: COLORS.rule, backgroundColor: COLORS.card, color: COLORS.ink }, className: "w-full rounded-lg border-2 p-3 text-xs leading-relaxed focus:outline-none focus:ring-2" }),
            React.createElement("button", { onClick: () => { importNote(selectedDeck.id, noteImportTitle, noteImportBody); setView('notes'); }, disabled: !noteImportTitle.trim() || !noteImportBody.trim(), style: { backgroundColor: noteImportTitle.trim() && noteImportBody.trim() ? COLORS.accent : COLORS.disabledBg, ...fontStyle }, className: "w-full rounded-xl py-3 text-white font-bold tracking-wide disabled:opacity-60" }, "Parse & Save")));
        // =====================================================================
        ...
        content = readingNote ? (React.createElement("div", { className: "flex flex-col gap-4" },
            ...
            React.createElement("div", { className: "rounded-2xl py-6 px-5 -mx-5", style: { backgroundColor: COLORS.card } },
