...
export default function FlashcardDrillApp() {
    ...
    const [view, setView] = useState('home'); // home | newDeck | import | deck | settings | practice | practiceAllModes | raceSetup
    ...
    // Markdown tables (.note-markdown table, injected via dangerouslySetInnerHTML —
    // see NoteBody/renderNoteHtml) get a custom horizontal-scroll UX, set up once
    // here via delegated listeners on `document` rather than per-note effects, so
    // it keeps working no matter which note/table is currently rendered:
    //  - the native scrollbar is hidden at rest and only shown for a moment after
    //    any scroll activity (touch swipe, wheel, or the drag below) — see the
    //    .fd-scrolling CSS toggle.
    //  - click-and-drag-to-scroll for mouse users, since a plain overflow-x:auto
    //    element doesn't support that natively (mouse drag normally just selects
    //    text). Touch/pen pointers are left untouched — the browser already
    //    handles swipe-scrolling on overflow-x:auto natively on mobile.
    useEffect(() => {
        const SCROLLING_CLASS = 'fd-scrolling';
        const SCROLL_HIDE_DELAY = 700;
        const scrollTimers = new WeakMap();
        function markScrolling(table) {
            table.classList.add(SCROLLING_CLASS);
            const prev = scrollTimers.get(table);
            if (prev)
                clearTimeout(prev);
            scrollTimers.set(table, setTimeout(() => {
                table.classList.remove(SCROLLING_CLASS);
                scrollTimers.delete(table);
            }, SCROLL_HIDE_DELAY));
        }
        // `scroll` doesn't bubble, but a capture-phase listener on document still
        // sees it fire on any descendant — this is how we delegate it without
        // attaching a listener to every table individually.
        function onScrollCapture(e) {
            const t = e.target;
            if (t && t.nodeType === 1 && t.tagName === 'TABLE' && t.closest && t.closest('.note-markdown')) {
                markScrolling(t);
            }
        }
        let dragTable = null;
        ...
        function onPointerDown(e) {
            if (e.pointerType && e.pointerType !== 'mouse')
                return; // let touch/pen scroll natively
            const table = e.target.closest && e.target.closest('.note-markdown table');
            if (!table)
                return;
            dragging = true;
            moved = false;
        ...
        function onPointerMove(e) {
            if (!dragging || !dragTable)
                return;
            const dx = e.clientX - startX;
            if (Math.abs(dx) > 3)
                moved = true;
            if (moved) {
                dragTable.scrollLeft = startScrollLeft - dx;
                e.preventDefault();
            }
        }
        function endDrag() {
            if (dragTable)
                dragTable.classList.remove('fd-dragging');
            dragging = false;
            dragTable = null;
            moved = false;
        }
    ...
    // createDeck: the two "Create Deck" buttons on the New Deck screen — one for
    // MCQs, one for Notes. `destination` ('cards' | 'notes') decides which import
    // screen the newly-created deck lands on.
    function createDeck(destination) {
    ...
    if (!loaded) {
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-3" },
            ...
            React.createElement("div", { className: "flex gap-2" },
                React.createElement("button", { onClick: () => setView('newDeck'), style: { borderColor: COLORS.ink, color: COLORS.ink, ...fontStyle }, className: "flex-1 rounded-xl border-2 py-2.5 font-bold flex items-center justify-center gap-1 text-xs focus:outline-none focus:ring-2" },
                ...
                React.createElement("button", { onClick: () => {
                        ...
                        setParsedPreview(null);
                        setView('import');
                    }, disabled: decks.length === 0, style: { borderColor: COLORS.accent, color: decks.length ? COLORS.accent : COLORS.inkFaint, ...fontStyle }, className: "flex-1 rounded-xl border-2 py-2.5 font-bold flex items-center justify-center gap-1 text-xs disabled:opacity-50 focus:outline-none focus:ring-2" },
                ...
                React.createElement("button", { onClick: () => {
                        ...
                        setNoteImportContentType('markdown');
                        setView('noteImport');
                    }, disabled: decks.length === 0, style: { borderColor: COLORS.solution, color: decks.length ? COLORS.solution : COLORS.inkFaint, ...fontStyle }, className: "flex-1 rounded-xl border-2 py-2.5 font-bold flex items-center justify-center gap-1 text-xs disabled:opacity-50 focus:outline-none focus:ring-2" },
            ...
            decks.length === 0 ? (React.createElement("p", { style: { ...fontStyle, color: COLORS.inkFaint }, className: "text-sm px-1 mt-2" }, "No decks yet. Create one to start building your first set of questions.")) : searchedDecks.length === 0 ? (React.createElement("p", { style: { ...fontStyle, color: COLORS.inkFaint }, className: "text-sm px-1 mt-2" },
                ...
                grouped[cat].map((deck) => {
                    ...
                    const dueNoteCount = deckNotes.filter((n) => n.sr && n.sr.due <= today).length;
                    const empty = deckCards.length === 0;
                    const emptyNotes = deckNotes.length === 0;
                    ...
                    const notePillLabel = emptyNotes ? 'empty' : dueNoteCount > 0 ? `${dueNoteCount} due` : 'up to date';
                    const notePillColor = emptyNotes ? COLORS.inkFaint : dueNoteCount > 0 ? COLORS.mustard : COLORS.green;
                    const notePillBg = emptyNotes ? COLORS.rule : dueNoteCount > 0 ? COLORS.mustardBg : COLORS.greenBg;
                    return (React.createElement("button", { key: deck.id, onClick: () => {
                            setSelectedDeckId(deck.id);
                            setView('deckChoice');
                        }, className: "w-full text-left rounded-xl border-2 px-4 py-3 flex items-start justify-between gap-2", style: { borderColor: COLORS.ink, backgroundColor: COLORS.card } },
                        React.createElement("div", { className: "min-w-0" },
                            React.createElement("div", { style: { ...fontStyle, color: COLORS.ink }, className: "font-bold" }, deck.name),
                            React.createElement("div", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs flex items-center flex-wrap gap-x-2 gap-y-0.5 mt-0.5" },
                                React.createElement("span", { className: "flex items-center gap-1" },
                                    React.createElement(Layers, { size: 11 }),
                                    deckCards.length,
                                    " card",
                                    deckCards.length !== 1 ? 's' : ''),
                                React.createElement("span", { className: "flex items-center gap-1" },
                                    React.createElement(FileText, { size: 11 }),
                                    deckNotes.length,
                                    " note",
                                    deckNotes.length !== 1 ? 's' : ''))),
                        React.createElement("div", { className: "flex flex-col items-end gap-1 shrink-0" },
                            React.createElement("span", { style: { ...monoStyle, color: pillColor, backgroundColor: pillBg }, className: "text-xs pl-1.5 pr-2 py-0.5 rounded-full flex items-center gap-1" },
                                React.createElement(Layers, { size: 10 }), pillLabel),
                            React.createElement("span", { style: { ...monoStyle, color: notePillColor, backgroundColor: notePillBg }, className: "text-xs pl-1.5 pr-2 py-0.5 rounded-full flex items-center gap-1" },
                                React.createElement(FileText, { size: 10 }), notePillLabel))));
                })))))),
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-4" },
            ...
            React.createElement("p", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs px-1" }, "Create, then add:"),
            React.createElement("div", { className: "flex gap-2" },
                React.createElement("button", { onClick: () => createDeck('cards'), disabled: !newDeckName.trim(), style: { backgroundColor: newDeckName.trim() ? COLORS.accent : COLORS.disabledBg, ...fontStyle }, className: "flex-1 rounded-xl py-3 text-white font-bold tracking-wide disabled:opacity-60 flex items-center justify-center gap-1.5" },
                    React.createElement(Layers, { size: 16 }),
                    " MCQs"),
                React.createElement("button", { onClick: () => createDeck('notes'), disabled: !newDeckName.trim(), style: { borderColor: COLORS.solution, color: COLORS.solutionText, backgroundColor: newDeckName.trim() ? COLORS.solutionBg : COLORS.disabledBg, ...fontStyle }, className: "flex-1 rounded-xl border-2 py-3 font-bold tracking-wide disabled:opacity-60 flex items-center justify-center gap-1.5" },
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-3" },
            ...
            React.createElement("button", { onClick: () => setView('deck'), className: "w-full rounded-2xl p-6 flex flex-col items-center gap-1.5 text-white", style: { backgroundColor: COLORS.accent } },
                React.createElement(Layers, { size: 22 }),
                React.createElement("span", { style: { ...fontStyle }, className: "text-lg font-bold" }, "Practice"),
                React.createElement("span", { style: { ...monoStyle }, className: "text-xs opacity-80" },
                    deckCardCount,
                    " card",
                    deckCardCount !== 1 ? 's' : '')),
            React.createElement("button", { onClick: () => setView('notes'), className: "w-full rounded-2xl border-2 p-6 flex flex-col items-center gap-1.5", style: { borderColor: COLORS.solution, backgroundColor: COLORS.solutionBg, color: COLORS.solutionText } },
                React.createElement(FileText, { size: 22 }),
                React.createElement("span", { style: { ...fontStyle }, className: "text-lg font-bold" }, "Notes"),
                React.createElement("span", { style: { ...monoStyle }, className: "text-xs opacity-80" },
                    deckNoteCount,
                    " note",
                    deckNoteCount !== 1 ? 's' : ''))));
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-3" },
            ...
            decks.length > 1 && (React.createElement("div", null,
                React.createElement("label", { style: { ...fontStyle, color: COLORS.ink }, className: "text-sm font-bold block mb-1" }, "Deck"),
                React.createElement("select", { value: selectedDeckId || '', onChange: (e) => setSelectedDeckId(e.target.value), style: { ...fontStyle, borderColor: COLORS.rule, backgroundColor: COLORS.card }, className: "w-full rounded-lg border-2 px-3 py-2 focus:outline-none focus:ring-2" }, decks.map((d) => (React.createElement("option", { key: d.id, value: d.id },
                    d.name,
                    d.category ? ` — ${d.category}` : '')))))),
    ...
    return (React.createElement("div", { style: { backgroundColor: COLORS.rule }, className: "w-full min-h-screen flex justify-center md:items-start md:py-10" },
        ...
        .note-markdown table::-webkit-scrollbar { height: 0px; background: transparent; }
        .note-markdown table.fd-scrolling { scrollbar-width: thin; }
        .note-markdown table.fd-scrolling::-webkit-scrollbar { height: 6px; }
        .note-markdown table.fd-scrolling::-webkit-scrollbar-track { background: transparent; }
        .note-markdown table.fd-scrolling::-webkit-scrollbar-thumb {
          background: ${COLORS.inkFaint}; border-radius: 4px;
        }
        .note-markdown table.fd-dragging { cursor: grabbing; }
        .note-markdown th, .note-markdown td {
          border: 1px solid ${COLORS.rule}; padding: 0.35em 0.6em;
          max-width: 24ch; white-space: normal; overflow-wrap: break-word; word-break: break-word;
