...
export default function FlashcardDrillApp() {
    ...
    function handleOptionClick(idx) {
        ...
        if (opt.correct) {
            ...
            setTimeout(() => {
                setFlashCorrectIdx(null);
                if (graduates) {
                    // First-try correct: this card won't reappear in any later review round
                    // this session, so its fate is sealed right now — save it immediately
                    // instead of waiting for the whole session to finish. Done here, after
                    // the flash, so the progress save doesn't re-render mid-animation and
                    // cut the green flash short.
                    const updated = cards.map((c) => (c.id === id ? { ...c, sr: scheduleNext(c.sr, !wasEverMissed) } : c));
    ...
    function deleteDeck(deckId) {
        ...
        triggerAutoBackup(nextDecks, nextCards);
    ...
    function saveEditCard() {
        ...
        const nextCards = cards.map((c) => c.id === editingCardId
            ? { ...c, question: p.question, correctAnswer: p.correctAnswer, distractors: p.distractors, hint: p.hint }
            : c);
        persistCards(nextCards);
    ...
    if (!loaded) {
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-4" },
            ...
            deckCards.length > 0 && (React.createElement("div", { className: "rounded-xl border-2 p-4 mt-2", style: { borderColor: COLORS.rule, backgroundColor: COLORS.card } },
                React.createElement("div", { style: fontStyle, className: "text-sm font-bold mb-2" }, "Edit Zone"),
                React.createElement("div", { className: "relative mb-3" },
                    ...
                    React.createElement("input", { type: "text", value: editZoneQuery, onChange: (e) => setEditZoneQuery(e.target.value), placeholder: "Search cards\u2026", style: { ...monoStyle, borderColor: COLORS.rule }, className: "w-full rounded-lg border-2 pl-8 pr-3 py-2 text-sm focus:outline-none focus:ring-2" })),
                React.createElement("div", { className: "flex flex-col gap-2 max-h-96 overflow-y-auto" },
                    deckCards
                        .filter((c) => c.question.toLowerCase().includes(editZoneQuery.trim().toLowerCase()))
                        .map((c) => (React.createElement("div", { key: c.id, className: "rounded-lg border p-2", style: { borderColor: COLORS.rule } }, editingCardId === c.id ? (React.createElement("div", { className: "flex flex-col gap-2" },
                        React.createElement("textarea", { value: editingCardDraft, onChange: (e) => setEditingCardDraft(e.target.value), rows: 6, style: { ...monoStyle, borderColor: COLORS.rule }, className: "w-full rounded-lg border-2 p-2 text-xs focus:outline-none focus:ring-2" }),
                        editingCardError && (React.createElement("p", { style: { ...monoStyle, color: COLORS.red }, className: "text-xs" }, editingCardError)),
                        React.createElement("div", { className: "flex gap-2" },
                            React.createElement("button", { onClick: saveEditCard, style: { backgroundColor: COLORS.ink, ...fontStyle }, className: "flex-1 rounded-lg text-white text-xs font-bold py-2" }, "Save"),
                        ...
                        React.createElement("p", { style: fontStyle, className: "text-xs text-slate-700" }, "Delete this card permanently?"),
                        React.createElement("div", { className: "flex gap-2" },
                        ...
                        React.createElement("span", { style: fontStyle, className: "text-xs text-slate-800 flex-1 truncate" }, c.question),
                    ...
                    deckCards.filter((c) => c.question.toLowerCase().includes(editZoneQuery.trim().toLowerCase())).length === 0 && (React.createElement("p", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs text-center py-2" }, "No cards match."))))),
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-4" },
            ...
            React.createElement("div", { className: "rounded-xl border-2 p-4", style: { borderColor: COLORS.rule, backgroundColor: COLORS.card } },
                ...
                React.createElement("p", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs mt-2" },
                    ...
                    " per cycle.")),
            React.createElement("div", { className: "rounded-xl border-2 p-4", style: { borderColor: COLORS.rule, backgroundColor: COLORS.card } },
                React.createElement("label", { style: fontStyle, className: "text-sm font-bold block mb-2" }, "Font Size"),
                React.createElement("div", { className: "flex items-center gap-2" },
                    ...
                    React.createElement("input", { type: "number", min: 12, max: 28, value: fontSizePx, onChange: (e) => {
                            ...
                            if (!Number.isNaN(v))
                                persistFontSize(v);
                        }, style: { ...monoStyle, borderColor: COLORS.rule }, className: "w-20 rounded-lg border-2 px-3 py-2 text-center focus:outline-none focus:ring-2" }),
                    React.createElement("button", { onClick: () => persistFontSize(fontSizePx + 1), style: { borderColor: COLORS.ink, color: COLORS.ink, ...fontStyle }, className: "w-10 h-10 rounded-lg border-2 font-bold text-lg" }, "+"),
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-4" },
            React.createElement(BackHeader, { onBack: () => { setView('settings'); setPendingRestore(null); setSignInSyncPrompt(null); }, title: "Backup & Restore" }),
            ...
            signInSyncPrompt && (React.createElement("div", { className: "rounded-xl border-2 p-4", style: { borderColor: COLORS.mustard, backgroundColor: COLORS.mustardBg } },
                ...
                React.createElement("div", { className: "flex gap-2" },
                    React.createElement("button", { onClick: applySignInMerge, style: { backgroundColor: COLORS.ink, ...fontStyle }, className: "flex-1 rounded-lg text-white text-sm font-bold py-2" }, "Keep & Merge"),
        ...
        if (phase === 'question' && currentCard) {
            ...
            content = (React.createElement(StampScreen, { tone: "green", title: "ALL CLEAR", subtitle: "every card answered correctly on first try \u2014 next reviews rescheduled", buttonLabel: "Done", onContinue: exitPractice }));
        }
    }
