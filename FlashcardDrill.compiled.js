...
import { Eye, EyeOff, Upload, Settings as SettingsIcon, ChevronLeft, Trash2, Plus, Search, Layers, FileText, Cloud, Download, RefreshCw, LogOut, Check, AlertTriangle, HardDriveDownload, Pencil, Brain, BookOpen, Clipboard, Home as HomeIcon, Moon, Sun } from 'lucide-react';
// =====================================================================
...
const COLORS_DARK = {
    ...
    mustardBg: '#3D3212',
    mustardText: '#E8C468',
    incorrect: '#C58CB5', // muted plum, hue-matched to light mode's incorrect
    incorrectBg: '#3A2A38',
    neutralBg: '#2D2D2D',
    disabledBg: '#3E3E3E',
    accent: '#627393', // muted navy, hue-matched to light mode's ink
    solution: '#749DC3', // muted steel blue, hue-matched to light mode's solution
    solutionBg: '#1E2D3D',
    solutionText: '#8FC1E9',
    cardsAccent: '#61AFD1', // muted cyan-blue, kept distinct from solution
...
// Auth is handled entirely server-side now, by the Cloudflare Pages Functions
// under /api/auth/* (see /functions/api/auth/*.js) — this file never holds a
// client ID/secret and never talks to Google directly for sign-in. See
// "Google sign-in persistence" in CONTEXT.md for why.
...
function renderNoteHtml(rawMarkdown) {
    ...
    // Wrap every rendered table in its own scroll viewport. marked's default table
    // renderer emits bare <table>/</table> with no attributes, and GFM markdown
    // tables can't nest, so open/close tags always pair up correctly here. The
    // wrapper (not the table) owns overflow-x/scrolling; the table itself stays
    // free to grow past the wrapper via min-width:max-content. See .note-table-wrap
    // CSS and NoteBody's JS below.
    html = html.replace(/<table>/g, '<div class="note-table-wrap"><table>').replace(/<\/table>/g, '</table></div>');
...
export default function FlashcardDrillApp() {
    ...
    const [activeCategory, setActiveCategory] = useState('All'); // Home screen category filter chips
    ...
    // succeeded on this browser. Kept for informational/telemetry purposes —
    // no longer gates whether a silent check is attempted (that used to matter
    // to avoid flashing Google's own UI; checkServerSession never shows any
    // Google UI at all, so there's nothing to avoid). Reset to false whenever
    // manual sign-out wipes storage. Added 2026-07-23, see CONTEXT.md.
    const [hasSignedInBefore, setHasSignedInBefore] = useState(false);
    const [driveAccessToken, setDriveAccessToken] = useState(null);
    // driveBackendMissing: true only if /api/auth/token comes back 404, meaning
    // the /functions/api/auth/* Pages Functions aren't deployed on this host —
    // surfaced as a setup notice on the Backup screen (see driveConfigured below).
    const [driveBackendMissing, setDriveBackendMissing] = useState(false);
    ...
    // pendingTokenRequestRef: the in-flight Promise for a /api/auth/token call, so
    // a mount-time check racing the 45-min background refresh (or online-reconnect)
    // reuses the same request instead of firing a second one.
    ...
    // Auth lives entirely behind the Cloudflare Pages Functions under /api/auth/*
    // (see /functions/api/auth/*.js) using a real OAuth authorization-code flow
    // with a server-held refresh token. The client never sees a client ID/secret
    // and never talks to Google for sign-in — it only ever calls its own
    // /api/auth/* endpoints. This is what makes "stay signed in" survive any
    // length of browser closure, not just under an hour — see "Google sign-in
    // persistence" in CONTEXT.md.
    // driveConfigured: false only if /api/auth/token has come back 404, meaning
    // the Functions aren't deployed on this host yet.
    function driveConfigured() {
        return !driveBackendMissing;
    }
    // fetchServerDriveToken: asks OUR backend for a fresh Google Drive access
    // token. The backend holds the refresh token — never the browser — and mints
    // a new ~1hr access token from it on every call, invisibly, with no Google UI
    // involved and no dependence on any browser-held Google session. Resolves to
    // null if there's no server session (never signed in, or signed out/expired).
    // Throws only on a genuine network/server error.
    async function fetchServerDriveToken() {
        let res;
        try {
        ...
        if (res.status === 404) {
            // /functions/api/auth/* isn't deployed on this host yet.
            setDriveBackendMissing(true);
            return null;
        }
        if (res.status === 401) {
            return null; // no session — never signed in, or signed out/revoked
        }
        if (!res.ok)
            throw new Error('token-fetch-failed');
        ...
        persistDriveTokenCache(data.access_token, Date.now() + expiresInSec * 1000);
        return data;
    }
    // getDriveAccessToken: every call site that used to call requestDriveToken()
    // now calls this. There's no popup/consent path here at all any more — that
    // only happens during the full-page redirect kicked off by handleGoogleSignIn
    // below. Dedupes concurrent callers so a mount check racing the 45-min
    // refresh timer doesn't fire two requests at once.
    async function getDriveAccessToken() {
        if (pendingTokenRequestRef.current)
            return pendingTokenRequestRef.current;
        const attempt = (async () => {
            const data = await fetchServerDriveToken();
            return data ? data.access_token : null;
        })();
        pendingTokenRequestRef.current = attempt.finally(() => {
    ...
    // asking our backend again — see tryReuseCachedDriveToken. Cleared entirely
    // (along with everything else) on manual sign-out, see handleGoogleSignOut.
    ...
    // screen) is clicked. A full top-level redirect to our backend's login
    // endpoint, which sends the browser on to Google's consent screen and back —
    // the page reloads on return; see the mount effect below (search "gdrive=")
    // for what happens next.
    function handleGoogleSignIn() {
        window.location.href = '/api/auth/login';
    }
    // handleGoogleSignOut: a manual sign-out wipes every local trace of this app —
    // decks/cards/notes, settings, and the sign-in info — from this browser, AND
    // tells the backend to delete the server-side session (refresh token + KV
    // entry) so this browser can't silently recover it later. Google Drive itself
    // is never touched (no delete call is made against the backup file), so the
    // next sign-in on any device can restore from it. Runs one last silent backup
    // first (best-effort, using the token we already have) so nothing created
    // since the last backup is lost once local storage is cleared below.
    async function handleGoogleSignOut() {
        setSignOutBusy(true);
        try {
            if (driveAccessToken) {
                try {
                    // Best-effort only — sign-out must never hang waiting on Drive.
                    await Promise.race([
            ...
            try {
                // Revokes the refresh token with Google and deletes the KV session —
                // see /functions/api/auth/logout.js. Best-effort: the local wipe
                // below still runs even if this fails or the network is down.
                await fetch('/api/auth/logout', { method: 'POST', credentials: 'same-origin' });
            }
            catch (e) { }
            // Record the explicit sign-out before attempting the wipe below, so that even
            // if the wipe only partly succeeds, checkServerSession still won't reuse the
    ...
    // acquisition. Works because /api/auth/login requests the (non-sensitive)
    // openid/email scopes alongside drive.appdata — see /functions/api/auth/login.js.
    ...
    // tryReuseCachedDriveToken: on load, before ever asking our backend for
    // anything, check whether we already have an access token saved from earlier
    // that hasn't expired yet (2 min safety buffer). If so, use it directly and
    // skip a network call entirely. Returns true if it reused a token.
    ...
    // checkServerSession: runs once when the app opens (on a normal reload — not
    // right after the OAuth redirect, which the mount effect below handles
    // separately), and again on 'online' after a connectivity gap. Asks our
    // backend for a token; no session just means "not signed in" — show
    // "Continue with Google" and move on, no error shown. Deliberately does NOT
    // run the sign-in sync/merge prompt — that's a one-time setup decision, only
    // triggered right after a fresh OAuth round trip.
    async function checkServerSession() {
        if (driveSignedOutByUser)
            return null;
        try {
            ...
            setGoogleSignedIn(true);
            persistBackupMeta({ hasSignedInBefore: true });
            captureGoogleEmailIfMissing(token);
            return token;
        }
        catch (e) {
            return null;
    ...
    async function driveBackupNow(silent, scope, overrideDecks, overrideCards, overrideNotes, overrideSettings, overrideToken) {
        ...
        try {
            ...
            try {
                ...
                // Token may have expired mid-session (~1hr lifetime) — our backend
                // mints a fresh one from the stored refresh token, no popup, always
                // available regardless of how long this tab's been open. Retry once
                // before giving up.
                token = await getDriveAccessToken();
                await attemptUpload(token);
            }
    ...
    async function syncAfterSignIn(token) {
        try {
            ...
            // fall back to driveAccessToken state: this can run synchronously right
            // after a setDriveAccessToken(token) call, before React has re-rendered —
            // so that state var may still be the PRE-sign-in value (null, or an old
            // token) in this closure. See "Fixed 2026-07-24" in CONTEXT.md.
    ...
    // Runs once per app load. Two cases:
    // 1. We just landed back here from the /api/auth/login → Google consent →
    //    /api/auth/callback round trip (see /functions/api/auth/*.js), marked by
    //    a "?gdrive=connected" (or "=error") query param on the redirect back.
    //    That's a genuine fresh sign-in, so run the full sync/merge flow, same as
    //    the old explicit-click path.
    // 2. A normal reload: first try a still-valid cached token (no network call
    //    at all — see tryReuseCachedDriveToken); otherwise ask our backend via
    //    checkServerSession — invisible, no Google UI involved, and unlike the
    //    old GIS-based silent reauth this works after ANY length of closure,
    //    since it's backed by a real server-side refresh token rather than a
    //    browser-held Google session.
    useEffect(() => {
        ...
        (async () => {
            ...
            if (gdrive === 'error') {
                setDriveNotice({ tone: 'error', text: 'Sign-in didn\u2019t complete. Please try \u201cContinue with Google\u201d again.' });
                return;
            }
            if (gdrive === 'connected') {
                const token = await fetchServerDriveToken();
                if (token && token.access_token) {
                    setDriveAccessToken(token.access_token);
                    setGoogleSignedIn(true);
                    persistBackupMeta({ driveSignedOutByUser: false, hasSignedInBefore: true });
                    await captureGoogleEmailIfMissing(token.access_token);
                    await syncAfterSignIn(token.access_token);
                    setDriveNotice((prev) => prev || { tone: 'success', text: 'Signed in.' });
                }
                else {
                    setDriveNotice({ tone: 'error', text: 'Sign-in didn\u2019t complete. Please try again.' });
                }
                return;
            }
    ...
    // Google's Drive access token expires after ~1hr regardless of how it was
    // obtained. If the tab is left open longer than that, nothing else renews
    // it — so proactively re-check with our backend every 45 minutes while
    // signed in, well before the token actually expires.
    ...
    if (!loaded) {
        ...
        const allCategoryKeys = Array.from(new Set(decks.map((d) => (d.category && d.category.trim()) ? d.category.trim() : 'Uncategorized'))).sort((a, b) => {
            if (a === 'Uncategorized')
                return 1;
            if (b === 'Uncategorized')
                return -1;
        ...
        const searchedDecks = decks.filter((d) => {
            const matchesQuery = !q || d.name.toLowerCase().includes(q) || (d.category || '').toLowerCase().includes(q);
            const deckCategory = (d.category && d.category.trim()) ? d.category.trim() : 'Uncategorized';
            const matchesCategory = activeCategory === 'All' || deckCategory === activeCategory;
        ...
        const homeNoticeColor = driveNotice?.tone === 'error' ? COLORS.red : driveNotice?.tone === 'success' ? COLORS.green : COLORS.mustardText;
        const homeNoticeBg = driveNotice?.tone === 'error' ? COLORS.redBg : driveNotice?.tone === 'success' ? COLORS.greenBg : COLORS.mustardBg;
        const totalNotesDue = notes.filter((n) => n.sr && n.sr.due <= today).length;
        const totalCardsAll = cards.length;
        const RING_R = 52;
        const RING_C = 2 * Math.PI * RING_R;
        const caughtUpFraction = totalCardsAll > 0 ? Math.max(0, (totalCardsAll - totalDue) / totalCardsAll) : 1;
        const ringOffset = RING_C * (1 - caughtUpFraction);
        const ringColor = totalDue > 0 ? COLORS.mustard : COLORS.green;
        const hour = new Date().getHours();
        const greeting = hour < 12 ? 'Good morning' : hour < 17 ? 'Good afternoon' : 'Good evening';
        // Signature element: a ring dial (fits the "spaced repetition" concept —
        // literally how much of today's review cycle is left) instead of flat
        // stat tiles. Fills via CSS custom properties + the .ring-progress
        // keyframe defined in SECTION: APP FRAME's <style> block.
        const ringEl = (React.createElement("div", { className: "relative shrink-0 w-28 h-28 lg:w-36 lg:h-36" },
            React.createElement("svg", { viewBox: "0 0 120 120", className: "w-full h-full" },
                React.createElement("circle", { cx: 60, cy: 60, r: RING_R, fill: "none", stroke: COLORS.rule, strokeWidth: 10 }),
                React.createElement("circle", { cx: 60, cy: 60, r: RING_R, fill: "none", stroke: ringColor, strokeWidth: 10, strokeLinecap: "round", className: "ring-progress", style: { strokeDasharray: RING_C, '--ring-from': RING_C, '--ring-to': ringOffset } })),
            React.createElement("div", { className: "absolute inset-0 flex flex-col items-center justify-center" }, totalDue > 0 ? (React.createElement(React.Fragment, null,
                React.createElement("span", { style: { ...fontStyle, color: COLORS.ink }, className: "text-3xl lg:text-4xl font-bold leading-none" }, totalDue),
        ...
        const heroEl = (React.createElement("div", { className: "flex flex-col gap-4 lg:flex-row lg:items-center lg:gap-8" },
            ...
            React.createElement("button", { onClick: () => {
                ...
                }, disabled: totalDue === 0, style: { backgroundColor: totalDue > 0 ? COLORS.accent : COLORS.disabledBg, ...fontStyle }, className: "w-full lg:w-auto lg:ml-auto rounded-xl py-3 px-6 text-white font-bold tracking-wide flex items-center justify-center gap-2 transition-transform active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-offset-2 lg:py-4 lg:text-base lg:rounded-2xl" },
                React.createElement(RefreshCw, { size: 16 }),
                "Review All Due (", totalDue, ")")));
        const noticeEl = driveNotice && (React.createElement("div", { className: "rounded-lg border-2 px-3 py-2 flex items-start gap-2", style: { borderColor: homeNoticeColor, backgroundColor: homeNoticeBg } },
        ...
        const actionRowEl = (React.createElement("div", { className: "flex gap-2 lg:gap-3" },
            React.createElement("button", { onClick: () => setView('newDeck'), style: { borderColor: COLORS.ink, color: COLORS.ink, ...fontStyle }, className: "flex-1 rounded-xl border-2 py-2.5 font-bold flex items-center justify-center gap-1 text-sm transition-transform active:scale-[0.98] focus:outline-none focus:ring-2 lg:py-3 lg:rounded-2xl" },
                React.createElement(Plus, { size: 16 }), " New Deck"),
            React.createElement("button", { onClick: () => {
                    if (!decks.length)
                        return;
                ...
                }, disabled: decks.length === 0, style: { borderColor: COLORS.ink, color: decks.length ? COLORS.ink : COLORS.inkFaint, ...fontStyle }, className: "flex-1 rounded-xl border-2 py-2.5 font-bold flex items-center justify-center gap-1 text-sm disabled:opacity-50 transition-transform active:scale-[0.98] focus:outline-none focus:ring-2 lg:py-3 lg:rounded-2xl" },
                React.createElement(Upload, { size: 16 }), " Import")));
        const searchEl = decks.length > 0 && (React.createElement("div", { className: "relative" },
        ...
        // Category filter chips: only worth showing once decks span more than
        // one category — otherwise it's a control with nothing to filter.
        const chipsEl = allCategoryKeys.length > 1 && (React.createElement("div", { className: "flex gap-2 overflow-x-auto pb-1 -mx-1 px-1" }, ['All', ...allCategoryKeys].map((cat) => {
            const isActive = activeCategory === cat;
            return (React.createElement("button", { key: cat, onClick: () => setActiveCategory(cat), style: { backgroundColor: isActive ? COLORS.accent : COLORS.card, color: isActive ? '#fff' : COLORS.ink, borderColor: isActive ? COLORS.accent : COLORS.rule, ...monoStyle }, className: "shrink-0 rounded-full border-2 px-3 py-1.5 text-xs font-bold whitespace-nowrap transition-transform active:scale-95" }, cat));
        })));
        const deckListEl = decks.length === 0 ? (React.createElement("p", { style: { ...fontStyle, color: COLORS.inkFaint }, className: "text-sm px-1 mt-2" }, "No decks yet. Create one to start building your first set of questions.")) : searchedDecks.length === 0 ? (React.createElement("p", { style: { ...fontStyle, color: COLORS.inkFaint }, className: "text-sm px-1 mt-2" },
            "No decks match \"", searchQuery, "\".")) : (React.createElement("div", { className: "flex flex-col gap-4 mt-1 lg:gap-6" }, categoryKeys.map((cat) => (React.createElement("div", { key: cat, className: "flex flex-col gap-2" },
            activeCategory === 'All' && (React.createElement("div", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs tracking-widest uppercase" }, cat)),
            React.createElement("div", { className: "grid grid-cols-1 gap-2 lg:grid-cols-2 lg:gap-3 xl:grid-cols-3" }, grouped[cat].map((deck) => {
                ...
                const readyPct = deckCards.length ? Math.round(((deckCards.length - dueCount) / deckCards.length) * 100) : 0;
                return (React.createElement("button", { key: deck.id, onClick: () => {
                        setSelectedDeckId(deck.id);
                        setView('deckChoice');
                    }, className: "w-full text-left rounded-xl border-2 px-4 py-3 flex flex-col gap-2 transition-transform duration-150 active:scale-[0.98] lg:p-5 lg:rounded-2xl lg:h-full lg:hover:-translate-y-0.5 lg:hover:shadow-lg shadow-sm", style: { borderColor: COLORS.ink, backgroundColor: COLORS.card } },
                    React.createElement("div", { className: "flex items-center justify-between gap-2" },
                        React.createElement("div", { style: { ...fontStyle, color: COLORS.ink }, className: "font-bold lg:text-base" }, deck.name),
                    ...
                    React.createElement("div", { style: { ...monoStyle }, className: "text-xs flex items-center gap-3" },
                        React.createElement("span", { className: "flex items-center gap-1", style: { color: COLORS.cardsAccent } }, React.createElement(Layers, { size: 12 }), deckCards.length, React.createElement("span", { style: { color: cardsDueColor } }, " ", dueCount, " due")),
                        React.createElement("span", { className: "flex items-center gap-1", style: { color: COLORS.notesAccent } }, React.createElement(BookOpen, { size: 12 }), deckNotes.length, React.createElement("span", { style: { color: notesDueColor } }, " ", noteDueCount, " due"))),
                    !empty && (React.createElement("div", { className: "h-1.5 rounded-full overflow-hidden", style: { backgroundColor: COLORS.rule } }, React.createElement("div", { className: "h-full rounded-full transition-all duration-500", style: { width: `${readyPct}%`, backgroundColor: dueCount > 0 ? COLORS.mustard : COLORS.green } })))));
            })))))));
        const driveEl = !googleSignedIn ? (React.createElement("button", { onClick: handleGoogleSignIn, disabled: driveBusy, style: { borderColor: COLORS.ink, color: COLORS.ink, ...fontStyle }, className: "w-full rounded-xl border-2 text-sm font-bold py-2.5 mt-2 flex items-center justify-center gap-2 disabled:opacity-50 transition-transform active:scale-[0.98]" },
            ...
            " \u00B7 Drive backup on"));
        content = (React.createElement("div", { className: "flex flex-col gap-4 lg:gap-6" },
            heroEl,
            noticeEl,
            actionRowEl,
        ...
        const deckCardsAll = cards.filter((c) => c.deckId === selectedDeck.id);
        const deckNotesAll = notes.filter((n) => n.deckId === selectedDeck.id);
        const dueCardCount = deckCardsAll.filter((c) => c.sr && c.sr.due <= today).length;
        const dueNoteCount = deckNotesAll.filter((n) => n.sr && n.sr.due <= today).length;
        const cardReadyPct = deckCardsAll.length ? Math.round(((deckCardsAll.length - dueCardCount) / deckCardsAll.length) * 100) : 0;
        const noteReadyPct = deckNotesAll.length ? Math.round(((deckNotesAll.length - dueNoteCount) / deckNotesAll.length) * 100) : 0;
        // choiceTile: shared Cards/Notes tile — reuses the same due-pill + thin
        // progress-bar language as Home's per-deck tiles (SECTION: HOME SCREEN)
        // instead of a bare icon+label, so "at a glance" status reads the same
        // way across the app. duration-200/300 (not Home's 500) to stay inside
        // this redesign's 150–300ms motion mandate.
        const choiceTile = ({ Icon, label, noun, accent, accentBg, count, dueCount, readyPct, onClick }) => {
            const empty = count === 0;
            const pillLabel = empty ? 'empty' : dueCount > 0 ? `${dueCount} due` : 'up to date';
            const pillColor = empty ? COLORS.inkFaint : dueCount > 0 ? COLORS.mustard : COLORS.green;
            const pillBg = empty ? COLORS.rule : dueCount > 0 ? COLORS.mustardBg : COLORS.greenBg;
            return (React.createElement("button", { onClick: onClick, className: "w-full text-left rounded-2xl border-2 p-5 flex flex-col gap-3 shadow-sm transition-transform duration-200 active:scale-[0.97] lg:p-8 lg:hover:-translate-y-0.5 lg:hover:shadow-lg", style: { borderColor: accent, backgroundColor: accentBg } },
                React.createElement("div", { className: "flex items-center justify-between gap-2" },
                    React.createElement("div", { className: "flex items-center gap-2" },
                        React.createElement(Icon, { size: 22, style: { color: accent } }),
                        React.createElement("span", { style: { ...fontStyle, color: accent }, className: "text-lg font-bold" }, label)),
                    React.createElement("span", { style: { ...monoStyle, color: pillColor, backgroundColor: pillBg }, className: "text-xs px-2 py-1 rounded-full shrink-0" }, pillLabel)),
                React.createElement("span", { style: { ...monoStyle, color: accent }, className: "text-xs" }, count, ` ${noun}${count !== 1 ? 's' : ''}`),
                !empty && (React.createElement("div", { className: "h-1.5 rounded-full overflow-hidden", style: { backgroundColor: COLORS.rule } }, React.createElement("div", { className: "h-full rounded-full transition-all duration-300", style: { width: `${readyPct}%`, backgroundColor: dueCount > 0 ? COLORS.mustard : COLORS.green } })))));
        };
        content = (React.createElement("div", { className: "flex flex-col gap-3" },
            React.createElement(BackHeader, { colors: COLORS, onBack: () => { setSelectedDeckId(null); setView('home'); }, title: selectedDeck.name }),
            React.createElement("div", { className: "flex flex-col gap-3 mt-1 lg:grid lg:grid-cols-2 lg:gap-4 lg:mt-2" },
                choiceTile({ Icon: Layers, label: 'Cards', noun: 'card', accent: COLORS.cardsAccent, accentBg: COLORS.cardsAccentBg, count: deckCardsAll.length, dueCount: dueCardCount, readyPct: cardReadyPct, onClick: () => setView('deck') }),
                choiceTile({ Icon: BookOpen, label: 'Notes', noun: 'note', accent: COLORS.notesAccent, accentBg: COLORS.notesAccentBg, count: deckNotesAll.length, dueCount: dueNoteCount, readyPct: noteReadyPct, onClick: () => setView('notes') }))));
        // =====================================================================
        // SECTION: DECK VIEW  (search: SECTION: DECK VIEW)
        // Redesigned 2026-07-20: Review Due — the only action here that writes to
        // sr/schedule (sessionAffectsProgress=true in beginSession) — lives in its
        // own tinted primaryCardEl card (mustard when cards are due, neutral once
        // caught up) so it visually outranks the read-only actions below it, which
        // keep plain/unfilled styling: Flip Through, View as Sheet, Import More,
        // then Flashcards Race's Practice All/Practice by Difficulty. The Practice
        // Order dropdown lives inside primaryCardEl and sets selectedDeck.practiceOrder,
        // which Review Due, Flip Through, and View as Sheet all read (defaults to
        // 'random' when unset). Practice All · MCQ and Practice by Difficulty each
        // have their own ephemeral, never-saved order picker (raceOrderChoice) —
        // see SECTION: FLASHCARDS RACE ENTRY SCREENS. A thin readyPct bar (same
        // motif as Home's per-deck tiles) sits under the card/due count line.
        // =====================================================================
    }
        ...
        const readyPct = deckCards.length ? Math.round(((deckCards.length - dueCards.length) / deckCards.length) * 100) : 0;
        // primaryCardEl: the one action that touches sr/schedule, set apart in its
        // own tinted card (mustard border+fill while cards are due, neutral rule/
        // card once caught up) so it reads as primary against the plain read-only
        // rows below — see SECTION: DECK VIEW comment above.
        const primaryCardEl = (React.createElement("div", { className: "rounded-2xl border-2 p-4 flex flex-col gap-3 shadow-sm transition-colors duration-300", style: { borderColor: dueCards.length ? COLORS.mustard : COLORS.rule, backgroundColor: dueCards.length ? COLORS.mustardBg : COLORS.card } },
            React.createElement("div", { style: { ...monoStyle, color: dueCards.length ? COLORS.mustardText : COLORS.inkFaint }, className: "text-xs tracking-widest uppercase" }, "Affects Your Schedule"),
            ...
            React.createElement("button", { onClick: () => beginSession(dueCards.map((c) => c.id), 'deck', `${selectedDeck.name} · Due`, true, deckOrder), disabled: dueCards.length === 0, style: { backgroundColor: dueCards.length ? COLORS.mustard : COLORS.disabledBg, ...fontStyle }, className: "w-full rounded-xl py-3.5 text-white font-bold tracking-wide disabled:opacity-60 flex items-center justify-center gap-2 transition-transform duration-200 active:scale-[0.98]" },
                React.createElement(RefreshCw, { size: 16 }),
                "Review Due (",
                dueCards.length,
                ")")));
        // studyRowEl / raceRowEl: read-only actions (no sr writes) — unfilled
        // outline styling, unchanged from before aside from tap feedback and
        // inline card counts, so they read as secondary next to primaryCardEl.
        const studyRowEl = (React.createElement("div", { className: "flex flex-col gap-3 lg:grid lg:grid-cols-2 lg:gap-3" },
            React.createElement("button", { onClick: () => startFlip(deckCards.map((c) => c.id), 'deck', deckOrder), disabled: deckCards.length === 0, style: { borderColor: COLORS.ink, color: COLORS.ink, ...fontStyle }, className: "w-full rounded-xl border-2 py-2.5 font-bold flex items-center justify-center gap-2 disabled:opacity-50 transition-transform duration-200 active:scale-[0.98]" },
                React.createElement(Layers, { size: 16 }),
                ` Flip Through (${deckCards.length})`),
            React.createElement("button", { onClick: () => { setSheetFilterIds(null); setView('sheet'); }, disabled: deckCards.length === 0, style: { borderColor: COLORS.ink, color: COLORS.ink, ...fontStyle }, className: "w-full rounded-xl border-2 py-2.5 font-bold flex items-center justify-center gap-2 disabled:opacity-50 transition-transform duration-200 active:scale-[0.98]" },
                React.createElement(FileText, { size: 16 }),
                ` View as Sheet (${deckCards.length})`)));
        const raceRowEl = (React.createElement("div", { className: "flex flex-col gap-3 lg:grid lg:grid-cols-2 lg:gap-3" },
            React.createElement("button", { onClick: () => { setRaceOrderChoice('random'); setView('practiceAllModes'); }, disabled: deckCards.length === 0, style: { backgroundColor: deckCards.length ? COLORS.accent : COLORS.disabledBg, ...fontStyle }, className: "w-full rounded-xl py-3 text-white font-bold tracking-wide disabled:opacity-60 transition-transform duration-200 active:scale-[0.98]" }, `Practice All \u00B7 MCQ (${deckCards.length})`),
            React.createElement("button", { onClick: () => { setRaceOrderChoice('random'); setSelectedDifficultyTier(null); setView('difficultyTiers'); }, disabled: deckCards.length === 0, style: { borderColor: COLORS.mustard, color: COLORS.mustard, ...fontStyle }, className: "w-full rounded-xl border-2 py-2.5 font-bold tracking-wide disabled:opacity-50 transition-transform duration-200 active:scale-[0.98]" }, "Practice by Difficulty")));
        content = (React.createElement("div", { className: "flex flex-col gap-5" },
            ...
            React.createElement("div", { className: "flex flex-col gap-1.5 -mt-2" },
                React.createElement("div", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs" },
                    ...
                    dueCards.length,
                    " due"),
                deckCards.length > 0 && (React.createElement("div", { className: "h-1.5 rounded-full overflow-hidden", style: { backgroundColor: COLORS.rule } }, React.createElement("div", { className: "h-full rounded-full transition-all duration-300", style: { width: `${readyPct}%`, backgroundColor: dueCards.length > 0 ? COLORS.mustard : COLORS.green } })))),
            primaryCardEl,
            React.createElement("div", { className: "flex flex-col gap-2" },
                React.createElement("div", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs tracking-widest uppercase" }, "Study Only \u00B7 No Scheduling Impact"),
                studyRowEl),
            React.createElement("button", { onClick: () => { setImportOrigin('deck'); setView('import'); }, style: { borderColor: COLORS.ink, color: COLORS.ink, ...fontStyle }, className: "w-full rounded-xl border-2 py-2.5 font-bold flex items-center justify-center gap-2 transition-transform duration-200 active:scale-[0.98]" },
                React.createElement(Upload, { size: 16 }),
                " Import More"),
            React.createElement("div", { className: "flex flex-col gap-2" },
                React.createElement("div", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs tracking-widest uppercase" }, "Flashcards Race \u00B7 No Scheduling Impact"),
                raceRowEl),
        ...
        // Redesigned 2026-07-20: a notes-count + due-count line and readyPct bar
        // (identical motif to Deck/Notes-tile elsewhere) now sit under BackHeader,
        // giving this list the same "at a glance" summary the Deck screen has for
        // cards. Row/button press feedback (active:scale) added; Due/HTML pill
        // padding unified with the rest of the app's due-pill convention.
        // =====================================================================
    }
    else if (view === 'notes' && selectedDeck) {
        const deckNotes = notes.filter((n) => n.deckId === selectedDeck.id);
        const todayStr2 = todayStr();
        const dueNoteCount = deckNotes.filter((n) => n.sr && n.sr.due <= todayStr2).length;
        const noteReadyPct = deckNotes.length ? Math.round(((deckNotes.length - dueNoteCount) / deckNotes.length) * 100) : 0;
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-3" },
            ...
            deckNotes.length > 0 && (React.createElement("div", { className: "flex flex-col gap-1.5 -mt-2" },
                React.createElement("div", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs" },
                    deckNotes.length,
                    " note",
                    deckNotes.length !== 1 ? 's' : '',
                    " \u00B7 ",
                    dueNoteCount,
                    " due"),
                React.createElement("div", { className: "h-1.5 rounded-full overflow-hidden", style: { backgroundColor: COLORS.rule } }, React.createElement("div", { className: "h-full rounded-full transition-all duration-300", style: { width: `${noteReadyPct}%`, backgroundColor: dueNoteCount > 0 ? COLORS.mustard : COLORS.green } })))),
            editZoneNotice && (React.createElement("div", { className: "rounded-lg border-2 px-3 py-2", style: { borderColor: COLORS.mustard, backgroundColor: COLORS.mustardBg } },
                React.createElement("span", { style: { ...monoStyle, color: COLORS.mustardText }, className: "text-xs" }, editZoneNotice))),
            React.createElement("input", { ref: noteFileInputRef, type: "file", accept: ".md,.markdown,.html,.htm", onChange: handleNoteFileSelected, className: "hidden" }),
            ...
            React.createElement("div", { className: "flex gap-2" },
                React.createElement("button", { onClick: () => { setImportOrigin('deck'); setNoteImportTitle(''); setNoteImportBody(''); setNoteImportContentType('markdown'); setView('noteImport'); }, style: { borderColor: COLORS.accent, color: COLORS.accent, ...fontStyle }, className: "flex-1 rounded-lg border-2 px-3 py-2 text-xs font-bold flex items-center justify-center gap-1 transition-transform duration-150 active:scale-[0.97]" },
                    React.createElement(Upload, { size: 14 }),
                    " Paste"),
                React.createElement("button", { onClick: () => { setImportOrigin('deck'); noteFileInputRef.current && noteFileInputRef.current.click(); }, style: { backgroundColor: COLORS.accent, ...fontStyle }, className: "flex-1 rounded-lg px-3 py-2 text-white text-xs font-bold flex items-center justify-center gap-1 transition-transform duration-150 active:scale-[0.97]" },
                    React.createElement(FileText, { size: 14 }),
                    " From File")),
            React.createElement("div", { className: "flex flex-col gap-2" },
                sortedNotes.map((n) => {
                    const isDue = n.sr && n.sr.due <= todayStr2;
                    return (React.createElement("button", { key: n.id, onClick: () => { setSelectedNoteId(n.id); setView('noteRead'); }, className: "w-full text-left rounded-xl border-2 px-4 py-3 flex items-center justify-between gap-2 shadow-sm transition-transform duration-150 active:scale-[0.98]", style: { borderColor: COLORS.rule, backgroundColor: COLORS.card } },
                        React.createElement("span", { style: { ...fontStyle, color: COLORS.ink }, className: "text-sm font-bold flex-1 truncate" }, n.title),
                        n.contentType === 'html' && (React.createElement("span", { style: { ...monoStyle, color: COLORS.ink, backgroundColor: COLORS.neutralBg }, className: "text-xs rounded-full px-2 py-1 shrink-0" }, "HTML")),
                        isDue && (React.createElement("span", { style: { ...monoStyle, color: COLORS.mustardText, backgroundColor: COLORS.mustardBg }, className: "text-xs rounded-full px-2 py-1 shrink-0" }, "Due"))));
                }),
        ...
        // Redesigned 2026-07-20: chrome only — title row gained a notesAccent
        // BookOpen icon (ties to the same violet Notes identity used on Deck
        // Choice/Home) and the body wrapper a shadow-sm; rating buttons got
        // press feedback. NoteBody/HtmlNoteFrame internals below are untouched.
        // =====================================================================
    }
        ...
        content = readingNote ? (React.createElement("div", { className: "flex flex-col gap-4" },
            ...
            React.createElement("div", { className: "rounded-2xl py-6 -mx-5 shadow-sm", style: { backgroundColor: COLORS.card } },
                React.createElement("div", { className: "flex items-center gap-2 mb-3 px-5" },
                    React.createElement(BookOpen, { size: 16, style: { color: COLORS.notesAccent } }),
                    React.createElement("h2", { style: { ...fontStyle, color: COLORS.ink }, className: "text-lg font-bold" }, readingNote.title)),
            ...
            React.createElement("p", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs text-center uppercase tracking-widest" }, "How was this read?"),
            React.createElement("div", { className: "flex gap-2" },
                React.createElement("button", { onClick: () => { rateNote(readingNote.id, 'hard'); setView('notes'); }, style: { backgroundColor: COLORS.red, ...fontStyle }, className: "flex-1 rounded-xl py-3 text-white text-sm font-bold shadow-sm transition-transform duration-150 active:scale-[0.97]" }, "Hard"),
                React.createElement("button", { onClick: () => { rateNote(readingNote.id, 'moderate'); setView('notes'); }, style: { backgroundColor: COLORS.mustard, ...fontStyle }, className: "flex-1 rounded-xl py-3 text-white text-sm font-bold shadow-sm transition-transform duration-150 active:scale-[0.97]" }, "Moderate"),
                React.createElement("button", { onClick: () => { rateNote(readingNote.id, 'easy'); setView('notes'); }, style: { backgroundColor: COLORS.green, ...fontStyle }, className: "flex-1 rounded-xl py-3 text-white text-sm font-bold shadow-sm transition-transform duration-150 active:scale-[0.97]" }, "Easy")))) : null;
        // =====================================================================
        // SECTION: FLIP SCREEN  (search: SECTION: FLIP SCREEN)
        // Tap-to-reveal card. Never affects scheduling or difficulty.
        // Redesigned 2026-07-20: added a thin progress bar under the header (same
        // track/fill language as Practice's), which fills through when a card's
        // answer is revealed, not just on advance. Card frame and Next Card
        // button gained shadow-sm/press feedback. Finished state still renders
        // the shared StampScreen component (defined near the bottom of the file,
        // just above SECTION: SESSION SUMMARY) — restyled once there, applies here too.
        // =====================================================================
    }
    else if (view === 'flip') {
        const flipCard = flipOrder.length && flipIndex < flipOrder.length ? cards.find((c) => c.id === flipOrder[flipIndex]) : null;
        if (flipCard) {
            // flipProgress counts the current card as "in progress" once its answer is
            // revealed, so the bar visibly ticks forward on each reveal rather than only
            // on advance — same 150–300ms fill motion as Practice's top bar.
            const flipProgress = flipOrder.length ? ((flipIndex + (flipShowAnswer ? 1 : 0)) / flipOrder.length) * 100 : 0;
            content = (React.createElement("div", { className: "flex flex-col gap-4" },
                React.createElement("div", { className: "flex justify-between items-center" },
                    React.createElement("button", { onClick: () => setView(returnView), style: { color: COLORS.inkFaint }, className: "p-1 rounded-full transition-transform duration-150 active:scale-90" },
                ...
                React.createElement("div", { className: "w-full h-1.5 rounded-full overflow-hidden", style: { backgroundColor: COLORS.rule } },
                    React.createElement("div", { className: "h-full rounded-full transition-all duration-300", style: { width: `${flipProgress}%`, backgroundColor: COLORS.accent } })),
                React.createElement("button", { onClick: () => !flipShowAnswer && setFlipShowAnswer(true), className: "w-full rounded-2xl border-2 px-5 py-8 text-left shadow-sm transition-transform duration-150 active:scale-[0.99] focus:outline-none focus:ring-2", style: { borderColor: COLORS.ink, backgroundColor: COLORS.card } },
                ...
                flipShowAnswer && (React.createElement("button", { onClick: () => {
                    ...
                    }, style: { backgroundColor: COLORS.accent, ...fontStyle }, className: "w-full rounded-xl py-3 text-white font-bold tracking-wide transition-transform duration-200 active:scale-[0.98]" }, "Next Card"))));
        }
        else {
        ...
        // Redesigned 2026-07-20: lowest-risk screen in this batch — no new
        // interactivity (still pure static scroll). Row rhythm only: the bare
        // "1." index prefix became a small rounded chip (matches the Practice
        // option-letter badge motif), rows gained shadow-sm and a touch more
        // padding, and each row's text column got min-w-0 as a defensive guard
        // against KaTeX math forcing row overflow (a flex item without min-w-0
        // can be forced wider than its container by long inline math — same
        // failure mode worth guarding against here even though it hasn't been
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-4" },
            ...
            sheetIds.length === 0 ? (React.createElement("p", { style: { ...fontStyle, color: COLORS.inkFaint }, className: "text-sm" }, "This deck is empty.")) : (React.createElement("div", { className: "flex flex-col gap-3.5" }, sheetIds.map((id, i) => {
                const c = cards.find((cc) => cc.id === id);
                if (!c)
                    return null;
                return (React.createElement("div", { key: id, className: "rounded-xl border-2 px-4 py-3.5 shadow-sm", style: { borderColor: COLORS.rule, backgroundColor: COLORS.card } },
                    React.createElement("div", { className: "flex items-start gap-2.5" },
                        React.createElement("span", { style: { ...monoStyle, color: COLORS.inkFaint, backgroundColor: COLORS.neutralBg }, className: "shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold" }, i + 1),
                        React.createElement("div", { className: "flex-1 min-w-0" },
                            React.createElement(MathText, { text: c.question, style: { ...fontStyle, color: COLORS.ink }, className: "text-sm font-medium block" }),
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-4" },
            ...
            React.createElement("div", { className: "rounded-xl border-2 p-4", style: { borderColor: COLORS.rule, backgroundColor: COLORS.card } },
                ...
                !driveConfigured() && (React.createElement("div", { className: "mt-4 rounded-lg border border-dashed px-3 py-2", style: { borderColor: COLORS.mustard, backgroundColor: COLORS.mustardBg } },
                    React.createElement("p", { style: { ...monoStyle, color: COLORS.mustardText }, className: "text-xs leading-relaxed" }, "Setup needed: this app's /api/auth backend isn't reachable here. Deploy the /functions folder alongside this site on Cloudflare Pages and set GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET and the KV binding \u2014 see DEPLOY.md."))))));
        // =====================================================================
        ...
        // Redesigned 2026-07-20: chrome only, per the design mandate — flash/shake/
        // race-urgent colors and the hint-toggle mechanism are untouched. Top progress
        // bar thickened slightly (h-1 → h-1.5) for more "how far am I" weight without
        // becoming a distraction. Question card frame gained shadow-sm. Option buttons'
        // rest state gained shadow-sm + a desktop-only hover lift (lg:hover:-translate-y
        // + shadow-md, guarded to only apply before an answer is picked) and tap
        // feedback (active:scale) — same lift/press language as Home's deck tiles and
        // Deck Choice's choiceTile(), not a new motif. Header back/hint-toggle buttons
        // gained the same tap feedback treatment.
        // =====================================================================
    }
        ...
        if (phase === 'question' && currentCard) {
            ...
            const answered = flashCorrectIdx !== null || flashWrongIdx !== null;
            content = (React.createElement("div", { className: "flex flex-col gap-4" },
                raceActive && (React.createElement("div", { className: "flex justify-between items-center rounded-xl border-2 px-3 py-2 shadow-sm transition-colors duration-200", style: { borderColor: raceUrgent ? COLORS.red : COLORS.ink, backgroundColor: raceUrgent ? COLORS.redBg : COLORS.card } },
                    ...
                    React.createElement("button", { onClick: endRace, style: { borderColor: COLORS.red, color: COLORS.red, ...fontStyle }, className: "rounded-lg border-2 px-3 py-1 text-xs font-bold transition-transform duration-150 active:scale-95" }, "End Race"))),
                React.createElement("div", { className: "flex justify-between items-center" },
                    React.createElement("button", { onClick: exitPractice, style: { color: COLORS.inkFaint }, className: "p-1 rounded-full transition-transform duration-150 active:scale-90" },
                        React.createElement(ChevronLeft, { size: 20 })),
                    React.createElement("div", { className: "text-center" },
                        React.createElement("div", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs tracking-widest" },
                            ...
                            "/",
                            cycleOrder.length)),
                    currentCard.hint ? (React.createElement("button", { onClick: () => setShowHint((h) => !h), style: { color: COLORS.mustard }, className: "p-1 rounded-full transition-transform duration-150 active:scale-90" }, showHint ? React.createElement(EyeOff, { size: 20 }) : React.createElement(Eye, { size: 20 }))) : React.createElement("div", { style: { width: 28 } })),
                React.createElement("div", { className: "w-full h-1.5 rounded-full overflow-hidden", style: { backgroundColor: COLORS.rule } },
                    React.createElement("div", { className: "h-full rounded-full transition-all duration-300", style: { width: `${progress}%`, backgroundColor: COLORS.accent } })),
                React.createElement("div", { className: "rounded-2xl border-2 px-5 py-7 shadow-sm", style: { borderColor: COLORS.ink, backgroundColor: COLORS.card } },
                ...
                showSolutionReveal ? (React.createElement("div", { className: "flex flex-col gap-3" },
                    React.createElement("div", { className: "rounded-xl border-2 px-4 py-3 shadow-sm", style: { borderColor: COLORS.solution, backgroundColor: COLORS.solutionBg } },
                    ...
                    React.createElement("button", { onClick: continueAfterSolution, style: { backgroundColor: COLORS.accent, ...fontStyle }, className: "w-full rounded-xl py-3 text-white font-bold tracking-wide transition-transform duration-200 active:scale-[0.98]" }, nextButtonLabel()))) : (React.createElement("div", { className: "flex flex-col gap-3" }, currentOptions.map((opt, i) => {
                    const isWrong = flashWrongIdx === i;
                    const isRight = flashCorrectIdx === i;
                    return (React.createElement("button", { key: i, onClick: () => handleOptionClick(i), disabled: flashCorrectIdx !== null || (raceActive && flashWrongIdx !== null), className: `w-full text-left rounded-xl border-2 px-4 py-3 flex items-center gap-3 shadow-sm transition-all duration-150 focus:outline-none focus:ring-2 active:scale-[0.98] ${!answered ? 'lg:hover:-translate-y-0.5 lg:hover:shadow-md' : ''} ${isWrong ? 'animate-shake' : ''}`, style: {
    ...
    // ===== SECTION: DESKTOP LAYOUT (search: SECTION: DESKTOP LAYOUT) =====
    // Below lg (1024px) the app is untouched: the original single centered card,
    // widened slightly at md for tablets. At lg+ a persistent sidebar (brand, Home/
    // Settings nav, New Deck, dark mode, Drive status) sits to the left and the
    // canvas to its right renders each screen in one of two treatments — a wide,
    // no-cap dashboard (Home only, for its stat tiles + deck grid) or a centered
    // "stage" panel sized to the screen's content (lists/forms get more room than
    // focused tasks like practice/flip/import). Edit stageWideViews to move a
    // screen between the two stage widths; mobile markup and behavior are unchanged.
    const isHomeView = view === 'home';
    const stageWideViews = ['deck', 'deckChoice', 'notes', 'editZone', 'sheet', 'settings', 'backup'];
    const stageMaxWidth = isHomeView ? 'lg:max-w-6xl' : stageWideViews.includes(view) ? 'lg:max-w-3xl' : 'lg:max-w-xl';
    const stagePanelClass = isHomeView ? '' : 'lg:rounded-3xl lg:border-2 lg:shadow-xl lg:p-10 lg:mt-2';
    const stagePanelStyle = isHomeView ? {} : { borderColor: COLORS.rule, backgroundColor: COLORS.paper };
    const isSettingsFamily = ['settings', 'editZone', 'backup'].includes(view);
    // sidebarEl: the lg+ desktop nav rail — brand/Home/Settings/New Deck, a live
    // Decks/Due mini-dashboard, dark mode toggle, and Drive status. Hidden below
    // lg (see its own "hidden lg:flex" className) so mobile/tablet render nothing
    // extra here at all.
    const sidebarEl = (React.createElement("aside", { style: { backgroundColor: COLORS.card, borderColor: COLORS.rule }, className: "hidden lg:flex lg:flex-col lg:w-72 lg:shrink-0 lg:h-screen lg:sticky lg:top-0 lg:px-6 lg:py-8 lg:gap-6 lg:border-r-2 lg:overflow-y-auto" },
        React.createElement("div", { onClick: goHome, className: "flex items-center gap-2.5 cursor-pointer" },
            React.createElement("div", { className: "w-9 h-9 rounded-xl flex items-center justify-center shrink-0", style: { backgroundColor: COLORS.accent } }, React.createElement(Layers, { size: 18, color: "#fff" })),
        ...
        React.createElement("nav", { className: "flex flex-col gap-1" },
            React.createElement("button", { onClick: goHome, style: { ...fontStyle, color: isHomeView ? '#fff' : COLORS.ink, backgroundColor: isHomeView ? COLORS.accent : 'transparent' }, className: "w-full text-left rounded-lg px-3 py-2.5 font-bold text-sm flex items-center gap-2.5" }, React.createElement(HomeIcon, { size: 16 }), "Home"),
            React.createElement("button", { onClick: () => setView('settings'), style: { ...fontStyle, color: isSettingsFamily ? '#fff' : COLORS.ink, backgroundColor: isSettingsFamily ? COLORS.accent : 'transparent' }, className: "w-full text-left rounded-lg px-3 py-2.5 font-bold text-sm flex items-center gap-2.5" }, React.createElement(SettingsIcon, { size: 16 }), "Settings"),
            React.createElement("button", { onClick: () => setView('newDeck'), style: { borderColor: COLORS.ink, color: COLORS.ink, ...fontStyle }, className: "w-full text-left rounded-lg px-3 py-2.5 font-bold text-sm flex items-center gap-2.5 border-2 mt-1" }, React.createElement(Plus, { size: 16 }), "New Deck")),
        (view === 'practice' && roundNumber > 1) ? (React.createElement("div", { className: "rounded-lg px-3 py-2", style: { backgroundColor: COLORS.mustardBg } }, React.createElement("span", { style: { ...monoStyle, color: COLORS.mustardText }, className: "text-xs font-bold" }, `In session \u00B7 Round ${roundNumber}`))) : null,
        React.createElement("div", { className: "flex-1" }),
        React.createElement("div", { className: "grid grid-cols-2 gap-2" },
            React.createElement("div", { className: "rounded-lg px-2.5 py-2", style: { backgroundColor: COLORS.neutralBg } }, React.createElement("div", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs uppercase tracking-wide" }, "Decks"), React.createElement("div", { style: { ...fontStyle, color: COLORS.ink }, className: "text-base font-bold" }, decks.length)),
            React.createElement("div", { className: "rounded-lg px-2.5 py-2", style: { backgroundColor: totalDue > 0 ? COLORS.mustardBg : COLORS.neutralBg } }, React.createElement("div", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs uppercase tracking-wide" }, "Due"), React.createElement("div", { style: { ...fontStyle, color: totalDue > 0 ? COLORS.mustard : COLORS.ink }, className: "text-base font-bold" }, totalDue))),
    ...
    const canvasEl = (React.createElement("div", { style: { backgroundColor: COLORS.paper, borderColor: COLORS.rule }, className: "w-full max-w-sm md:max-w-lg lg:max-w-none min-h-screen md:min-h-0 md:rounded-2xl md:border-2 md:shadow-2xl lg:rounded-none lg:border-0 lg:shadow-none lg:flex-1 lg:flex lg:justify-center lg:overflow-y-auto lg:px-12 lg:py-12" },
        React.createElement("div", { className: `w-full max-w-sm mx-auto px-5 py-6 lg:mx-0 lg:w-full lg:px-0 lg:py-0 lg:self-start ${stageMaxWidth}` },
            mobileHeaderEl,
            React.createElement("div", { className: stagePanelClass, style: stagePanelStyle }, content || fallbackEl))));
    return (React.createElement("div", { style: { backgroundColor: COLORS.rule }, className: "w-full min-h-screen flex justify-center md:items-start md:py-10 lg:justify-start lg:items-stretch lg:py-0" },
        ...
        .ring-progress {
          animation: ringFill 0.9s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
        ...
        @media (prefers-reduced-motion: reduce) {
          ...
          .ring-progress { animation: none !important; stroke-dashoffset: var(--ring-to) !important; }
        }
        ...
        .note-markdown table {
          min-width: max-content;   /* lets column widths follow real content needs
                                        (up to each cell's max-width) instead of
                                        shrinking to fit the wrapper. Safe here because
                                        the WRAPPER, not the table, owns overflow-x —
                                        the table is free to grow past it and get
                                        scrolled, instead of growing and having nothing
                                        left to scroll (that was the actual bug). */
          border-collapse: collapse;
          font-size: 0.9em;
          table-layout: auto;
        }
        .note-markdown .note-table-wrap::-webkit-scrollbar { height: 6px; }
        .note-markdown .note-table-wrap::-webkit-scrollbar-track { background: transparent; }
        .note-markdown .note-table-wrap::-webkit-scrollbar-thumb { background: transparent; border-radius: 3px; }
        .note-markdown .note-table-wrap.fd-scrolling { scrollbar-width: thin; scrollbar-color: ${COLORS.inkFaint} transparent; }
        .note-markdown .note-table-wrap.fd-scrolling::-webkit-scrollbar-thumb { background: ${COLORS.inkFaint}; }
        ...
        .note-markdown th, .note-markdown td {
          ...
          /* Deliberately NO word-break/overflow-wrap: those make the browser treat a
             ...
             min-width:60px only stops a 1-2 char column collapsing to near-zero — it's
             deliberately small so it never competes with the table's own
             min-width:max-content for space; do not raise this to "fix" narrow
             columns, that reintroduces the uniform-collapse bug (see CONTEXT.md). */
        }
...
// Every rendered <table> is wrapped in a .note-table-wrap div by renderNoteHtml;
// the WRAPPER (not the table) is the scroll/overflow element, so the table itself
// stays a native <table> free to grow past the wrapper via min-width:max-content.
...
// overflow-x:auto). Dragging is skipped entirely when a wrapper doesn't overflow,
...
function NoteBody({ text, className, style }) {
    ...
    useEffect(() => {
        ...
        if (!container)
            return;
        const wraps = Array.from(container.querySelectorAll('.note-table-wrap'));
        const cleanups = [];
        wraps.forEach((wrap) => {
            let isDown = false;
            let startX = 0;
            ...
            let hideTimer = null;
            const showScrollbar = () => {
                wrap.classList.add('fd-scrolling');
                if (hideTimer)
                    clearTimeout(hideTimer);
                hideTimer = setTimeout(() => wrap.classList.remove('fd-scrolling'), 700);
            };
            ...
            const endDrag = () => {
                if (!isDown)
                    return;
                isDown = false;
                wrap.classList.remove('fd-dragging');
            };
            const onMouseDown = (e) => {
                ...
                wrap.classList.add('fd-dragging');
                e.preventDefault();
            };
...
function StampScreen({ tone, title, subtitle, buttonLabel, onContinue, colors: COLORS }) {
    ...
    // tone-matched icon reinforces the postmark's meaning at a glance: mustard (round
    // complete, cards flagged for review) gets a warning triangle, navy (clean cycle/
    // deck-complete) gets a check — same Check/AlertTriangle already imported and used
    // for Drive notice tones (Home screen, Backup screen).
    const ToneIcon = tone === 'mustard' ? AlertTriangle : Check;
    return (React.createElement("div", { className: "flex flex-col items-center justify-center gap-7 py-16" },
        React.createElement("div", { className: "stamp-el rounded-lg px-7 py-6 text-center flex flex-col items-center gap-2 shadow-sm", style: { border: `4px double ${toneColor}`, backgroundColor: toneBg, transform: 'rotate(-4deg)' } },
            React.createElement(ToneIcon, { size: 20, style: { color: toneColor } }),
        ...
        React.createElement("button", { onClick: onContinue, style: { backgroundColor: toneColor, fontFamily: "'Roboto Slab', serif" }, className: "w-full max-w-xs rounded-xl py-3 text-white font-bold tracking-wide transition-transform duration-200 active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-offset-2" }, buttonLabel)));
}
...
//
// Redesigned 2026-07-20: the sticky tier-tile row and the fixed Done bar keep their
// exact positioning/z-index/scroll behavior — only fill/border/type changed, plus a
// thin per-tile progress bar (counts[t.key]/items.length, same track/fill language as
// everywhere else) under each tile's label+count row; tap-to-filter and the dim/active
// opacity toggle are byte-for-byte the same logic as before. Per-card rows gained
// shadow-sm and their Hint/Solution toggle pills gained press feedback (active:scale) —
// the one interactivity addition explicitly in scope here. The ALL CLEAR banner (not
// part of the sticky row, scrolls away normally) got the same tone-icon-in-stamp
// treatment as StampScreen above, for visual consistency between the two.
function SessionSummaryScreen({ colors: COLORS, cards, sessionPoolIds, sessionResults, sessionAffectsProgress, raceMode, onDone }) {
    ...
    return (React.createElement("div", { className: "flex flex-col gap-4 -mb-4" },
        React.createElement("div", { className: "flex flex-col items-center gap-1 pt-2" },
            React.createElement("div", { className: "stamp-el rounded-lg px-7 py-5 text-center flex flex-col items-center gap-1.5 shadow-sm", style: { border: `4px double ${COLORS.green}`, backgroundColor: COLORS.greenBg, transform: 'rotate(-4deg)' } },
        ...
        React.createElement("div", { className: "flex flex-col gap-2" }, visibleItems.map((it) => {
            ...
            return (React.createElement("div", { key: it.id, className: "rounded-xl border-2 px-4 py-3.5 shadow-sm", style: { borderColor: COLORS.rule, backgroundColor: COLORS.card } },
                React.createElement("div", { className: "flex items-start justify-between gap-2" },
                ...
                React.createElement("div", { className: "flex items-center gap-1.5 flex-wrap mt-1.5" },
                    ...
                    it.hint ? (React.createElement("button", { key: "hintBtn", onClick: () => toggleHint(it.id), style: { borderColor: COLORS.mustard, color: COLORS.mustard }, className: "inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border transition-transform duration-150 active:scale-90" },
                        openHints.has(it.id) ? React.createElement(EyeOff, { size: 10 }) : React.createElement(Eye, { size: 10 }),
                        "Hint")) : null,
                    it.solution ? (React.createElement("button", { key: "solutionBtn", onClick: () => toggleSolution(it.id), style: { borderColor: COLORS.solution, color: COLORS.solution }, className: "inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border transition-transform duration-150 active:scale-90" },
        ...
        React.createElement("div", { className: "fixed inset-x-0 bottom-0 z-20 flex justify-center pointer-events-none", style: { bottom: `${chromeInsetPx}px` } },
            React.createElement("div", { className: "done-bar w-full max-w-sm md:max-w-lg pointer-events-auto", style: {
                ...
                React.createElement("div", { className: "w-full max-w-sm mx-auto px-5 pt-3", style: { paddingBottom: 'calc(0.75rem + env(safe-area-inset-bottom, 0px))' } },
                    React.createElement("button", { onClick: onDone, style: { backgroundColor: COLORS.green, fontFamily: "'Roboto Slab', serif" }, className: "w-full rounded-xl py-3 text-white font-bold tracking-wide flex items-center justify-center gap-2" },
