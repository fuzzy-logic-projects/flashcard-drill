...
import { Eye, EyeOff, Upload, Settings as SettingsIcon, ChevronLeft, Trash2, Plus, Search, Layers, FileText, Cloud, Download, RefreshCw, LogOut, Check, AlertTriangle, HardDriveDownload, Pencil, Brain, BookOpen } from 'lucide-react';
// =====================================================================
...
export default function FlashcardDrillApp() {
    ...
    const [view, setView] = useState('home'); // home | newDeck | import | deck | settings | practice | raceModeChoice | raceDuration | race
    ...
    // ===== Flashcards Race state (search: FLASHCARDS RACE) =====
    // raceSourcePoolIds/raceSourceLabel/raceSourceReturnView: what a Practice All ·
    // MCQ / Practice by Difficulty tap resolves to, carried into the mode-choice
    // screen ('raceModeChoice') where the person picks Normal Practice or Race.
    const [raceSourcePoolIds, setRaceSourcePoolIds] = useState([]);
    const [raceSourceLabel, setRaceSourceLabel] = useState('');
    const [raceSourceReturnView, setRaceSourceReturnView] = useState('deck');
    const [raceModePracticeOrder, setRaceModePracticeOrder] = useState('random'); // dropdown on the mode-choice screen, fresh every visit — never persisted
    const [raceDurationDraft, setRaceDurationDraft] = useState('120');
    const [raceTotalSeconds, setRaceTotalSeconds] = useState(120);
    const [raceSecondsLeft, setRaceSecondsLeft] = useState(0);
    const [raceRunning, setRaceRunning] = useState(false);
    const [raceOrder, setRaceOrder] = useState([]); // ordered card ids for this race, fixed for the whole race
    const [racePos, setRacePos] = useState(0);
    const [raceOptions, setRaceOptions] = useState([]);
    const [raceResults, setRaceResults] = useState({}); // { [cardId]: 'strong'|'good'|'weak'|'incorrect' } — one pass only, set the instant a card is answered
    const [raceFlashWrongIdx, setRaceFlashWrongIdx] = useState(null);
    const [raceFlashCorrectIdx, setRaceFlashCorrectIdx] = useState(null);
    const raceTimerRef = useRef(null);
    const raceQuestionShownAtRef = useRef(null);
    useEffect(() => {
        (async () => {
            try {
    ...
    function downloadLocalBackup() {
        const payload = buildBackupPayload(decks, cards, notes, { cycleSize, practiceOrder });
    ...
    async function driveBackupNow(silent, overrideDecks, overrideCards, overrideNotes) {
        ...
        try {
            ...
            const payload = buildBackupPayload(overrideDecks || decks, overrideCards || cards, overrideNotes || notes, { cycleSize, practiceOrder });
    ...
    // sessionOrderRef: the practice-order value the CURRENT session should use for every
    // round, including review rounds — set once in beginSession, read by startRound so a
    // startReviewRound() call later (which doesn't know the caller's order) stays consistent.
    const sessionOrderRef = useRef('random');
    ...
    function startRound(poolIds, round) {
        const ordered = orderPool(poolIds, sessionOrderRef.current, cards);
    ...
    // beginSession: entry point for ANY practice session. affectsProgress=false is what makes
    // "Practice by Difficulty" read-only (no SR change, no difficulty change, no backup).
    // order: practice order to use for this session (defaults to the deck's own setting via
    // callers) — falls back to 'random' if omitted.
    function beginSession(poolIds, returnTo, label, affectsProgress, order) {
        if (!poolIds.length)
            return;
        sessionOrderRef.current = order || 'random';
    ...
    // Entirely separate from the MCQ practice engine above — never touches sr/difficulty,
    // never persists cards, never triggers a Drive backup, and is rebuilt fresh (via
    // raceOrder/raceResults being reset in startRace) every time this section opens.
    // One pass only: a card is answered once (right or wrong) and never comes back.
    // startRace: begins a race session with a given pool/order/duration.
    function startRace(poolIds, order, totalSeconds) {
        if (!poolIds.length)
            return;
        const ordered = orderPool(poolIds, order || 'random', cards);
        setRaceOrder(ordered);
        setRacePos(0);
        setRaceResults({});
        setRaceTotalSeconds(totalSeconds);
        setRaceSecondsLeft(totalSeconds);
        setRaceRunning(true);
        setRaceFlashWrongIdx(null);
        setRaceFlashCorrectIdx(null);
        setView('race');
    }
    // Countdown timer — ticks every second while raceRunning, ends the race at 0.
    useEffect(() => {
        if (view !== 'race' || !raceRunning)
            return;
        raceTimerRef.current = setInterval(() => {
            setRaceSecondsLeft((s) => {
                if (s <= 1) {
                    setRaceRunning(false);
                    return 0;
                }
    ...
    // Builds fresh shuffled options + resets the per-question timer whenever the race
    // question changes — mirrors the MCQ options-regeneration effect above.
    useEffect(() => {
        if (view !== 'race' || !raceRunning)
            return;
        const id = raceOrder[racePos];
        const card = cardsRef.current.find((c) => c.id === id);
        if (!card)
            return;
        const opts = shuffle([
        ...
        setRaceFlashWrongIdx(null);
        setRaceFlashCorrectIdx(null);
        raceQuestionShownAtRef.current = Date.now();
    }, [view, raceRunning, racePos, raceOrder]);
    // handleRaceOptionClick: one-pass answer — right or wrong, the card is scored once and
    // the race moves on to the next card immediately (no requeueing, no retries).
    function handleRaceOptionClick(idx) {
        if (!raceRunning)
            return;
        const opt = raceOptions[idx];
        if (!opt)
            return;
        const id = raceOrder[racePos];
        if (opt.correct) {
            const card = cardsRef.current.find((c) => c.id === id);
            const elapsedSeconds = raceQuestionShownAtRef.current ? (Date.now() - raceQuestionShownAtRef.current) / 1000 : 0;
            const tier = card ? (classifyDifficulty(card, elapsedSeconds, decks.find((d) => d.id === card.deckId)) || 'good') : 'good';
            setRaceFlashCorrectIdx(idx);
            setTimeout(() => {
                setRaceResults((prev) => ({ ...prev, [id]: tier }));
                advanceRace();
            }, 300);
        }
        else {
            setRaceFlashWrongIdx(idx);
            setTimeout(() => {
                setRaceResults((prev) => ({ ...prev, [id]: 'incorrect' }));
                advanceRace();
            }, 350);
        }
    }
    // advanceRace: moves to the next card, or ends the race if that was the last one.
    function advanceRace() {
        const nextPos = racePos + 1;
        if (nextPos < raceOrder.length) {
            setRacePos(nextPos);
        }
        else {
            setRaceRunning(false);
    ...
    // endRaceNow: the manual "End Race" button — stops the timer immediately, whatever's
    // left in the pool (including the card on screen, if unanswered) becomes "Never Fought".
    function endRaceNow() {
        setRaceRunning(false);
    }
    // exitRace: leaves the race screens back to wherever the mode-choice screen was opened
    // from. No backup flush — races never touch persisted data.
    function exitRace() {
        setView(raceSourceReturnView);
    }
    ...
    function handleOptionClick(idx) {
        ...
        if (opt.correct) {
            ...
            setTimeout(() => {
                ...
                // Practice by Difficulty) still get a Strong/Good/Weak/Incorrect breakdown
                // in the end-of-session summary. Only sessionAffectsProgress persists it
                // back to the card's stored difficulty/sr — see the block below.
    ...
    // saveCycleSize: the "Cards per cycle" control in Settings, affecting how practice sessions are batched.
    ...
    // chooseDeckPracticeOrder: the dropdown just below "Spaced Repetition · MCQ" in
    // Deck > Practice. Mirrors the Notes sort dropdown, but persisted onto the deck
    // record itself (not global Settings) and auto-backed-up like any other deck edit.
    // Defaults to 'random' for any deck that predates this field.
    function chooseDeckPracticeOrder(deckId, order) {
    ...
    if (!loaded) {
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-3" },
            ...
            React.createElement("button", { onClick: () => setView('notes'), className: "w-full rounded-2xl border-2 p-6 flex flex-col items-center gap-1", style: { borderColor: COLORS.mustard, backgroundColor: COLORS.mustardBg } },
                React.createElement("span", { style: { ...fontStyle, color: COLORS.mustardText }, className: "text-lg font-bold" }, "Notes"),
                React.createElement("span", { style: { ...monoStyle, color: COLORS.mustardText }, className: "text-xs" },
                    deckNoteCount,
                    " note",
                    deckNoteCount !== 1 ? 's' : '')),
            React.createElement("button", { onClick: () => setView('deck'), className: "w-full rounded-2xl border-2 p-6 flex flex-col items-center gap-1", style: { borderColor: COLORS.ink, backgroundColor: COLORS.card } },
                React.createElement("span", { style: { ...fontStyle, color: COLORS.ink }, className: "text-lg font-bold" }, "Practice"),
                React.createElement("span", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs" },
                    deckCardCount,
                    " card",
                    deckCardCount !== 1 ? 's' : ''))));
        // =====================================================================
        // SECTION: DECK VIEW  (search: SECTION: DECK VIEW)
        // Review Due / Practice Order dropdown / Flip Through / View as Sheet /
        // Import More / Flashcards Race (Practice All · MCQ, Practice by
        // Difficulty) buttons, in that order top to bottom. Button fill color
        // for the main button: COLORS.mustard. To reorder buttons, move their
        // <button> blocks below.
        // =====================================================================
    }
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-4" },
            ...
            React.createElement("button", { onClick: () => beginSession(dueCards.map((c) => c.id), 'deck', `${selectedDeck.name} · Due`, true, deckOrder), disabled: dueCards.length === 0, style: { backgroundColor: dueCards.length ? COLORS.mustard : COLORS.disabledBg, ...fontStyle }, className: "w-full rounded-xl py-3 text-white font-bold tracking-wide disabled:opacity-60" },
                ...
                dueCards.length,
                ")"),
            React.createElement("select", { value: deckOrder, onChange: (e) => chooseDeckPracticeOrder(selectedDeck.id, e.target.value), style: { ...monoStyle, borderColor: COLORS.rule, backgroundColor: COLORS.card, color: COLORS.ink }, className: "w-full rounded-lg border-2 px-2 py-2 text-xs focus:outline-none focus:ring-2" },
            ...
            React.createElement("button", { onClick: () => {
                    setRaceSourcePoolIds(deckCards.map((c) => c.id));
                    setRaceSourceLabel(selectedDeck.name);
                    setRaceSourceReturnView('deck');
                    setRaceModePracticeOrder('random');
                    setView('raceModeChoice');
                }, disabled: deckCards.length === 0, style: { backgroundColor: deckCards.length ? COLORS.accent : COLORS.disabledBg, ...fontStyle }, className: "w-full rounded-xl py-3 text-white font-bold tracking-wide disabled:opacity-60" }, "Practice All \u00B7 MCQ"),
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-3" },
            ...
            React.createElement("div", { className: "flex gap-2" },
                React.createElement("button", { onClick: () => { setNoteImportTitle(''); setNoteImportBody(''); setNoteImportContentType('markdown'); setView('noteImport'); }, style: { borderColor: COLORS.accent, color: COLORS.accent, ...fontStyle }, className: "flex-1 rounded-lg border-2 px-3 py-2 text-xs font-bold flex items-center justify-center gap-1" },
                    React.createElement(Upload, { size: 14 }),
                    " Paste"),
                React.createElement("button", { onClick: () => noteFileInputRef.current && noteFileInputRef.current.click(), style: { backgroundColor: COLORS.accent, ...fontStyle }, className: "flex-1 rounded-lg px-3 py-2 text-white text-xs font-bold flex items-center justify-center gap-1" },
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-3" },
            React.createElement(BackHeader, { colors: COLORS, onBack: () => setView('notes'), title: noteImportContentType === 'html' ? 'Import HTML Note' : 'Import Note' }),
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-4" },
            React.createElement(BackHeader, { colors: COLORS, onBack: () => { setSheetFilterIds(null); setView('deck'); }, title: `${selectedDeck.name} · Sheet` }),
        ...
        // The Strong / Good / Weak / Incorrect picker. Tile colors (same mapping
        // as Session Summary — kept in sync, both arrays edited together):
        // Strong=COLORS.green/greenBg, Good=COLORS.incorrect/incorrectBg,
        // Weak=COLORS.mustard/mustardBg, Incorrect=COLORS.red/redBg.
        // =====================================================================
    }
        ...
        content = (React.createElement("div", { className: "flex flex-col gap-3" },
            ...
            tiers.map((t) => (React.createElement("button", { key: t.key, onClick: () => {
                    setSelectedDifficultyTier(t.key);
                    setRaceSourcePoolIds(tierCards[t.key].map((c) => c.id));
                    setRaceSourceLabel(`${selectedDeck.name} · ${t.label}`);
                    setRaceSourceReturnView('difficultyTiers');
                    setRaceModePracticeOrder('random');
                    setView('raceModeChoice');
                }, disabled: tierCards[t.key].length === 0, className: "w-full rounded-xl border-2 p-4 flex items-center justify-between disabled:opacity-40", style: { borderColor: t.color, backgroundColor: t.bg } },
        ...
        // =====================================================================
        // SECTION: RACE MODE CHOICE SCREEN  (search: SECTION: RACE MODE CHOICE)
        // Reached from Practice All · MCQ or a Practice by Difficulty tier.
        // Practice-order dropdown (mirrors Deck > Practice's, but local to this
        // screen only — never persisted, always resets to Random on entry) +
        // Normal Practice / Race buttons.
        // =====================================================================
    }
    else if (view === 'raceModeChoice' && selectedDeck) {
        content = (React.createElement("div", { className: "flex flex-col gap-3" },
            React.createElement(BackHeader, { colors: COLORS, onBack: () => setView(raceSourceReturnView), title: raceSourceLabel }),
            React.createElement("p", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs -mt-1" },
                raceSourcePoolIds.length,
                " card",
                raceSourcePoolIds.length !== 1 ? 's' : ''),
            React.createElement("select", { value: raceModePracticeOrder, onChange: (e) => setRaceModePracticeOrder(e.target.value), style: { ...monoStyle, borderColor: COLORS.rule, backgroundColor: COLORS.card, color: COLORS.ink }, className: "w-full rounded-lg border-2 px-2 py-2 text-xs focus:outline-none focus:ring-2" },
                React.createElement("option", { value: "random" }, "Random Order"),
                React.createElement("option", { value: "new" }, "New Cards First"),
                React.createElement("option", { value: "old" }, "Old Cards First"),
                React.createElement("option", { value: "difficult" }, "Difficult Cards First")),
            React.createElement("button", { onClick: () => beginSession(raceSourcePoolIds, raceSourceReturnView, raceSourceLabel, false, raceModePracticeOrder), disabled: raceSourcePoolIds.length === 0, style: { backgroundColor: raceSourcePoolIds.length ? COLORS.accent : COLORS.disabledBg, ...fontStyle }, className: "w-full rounded-xl py-3 text-white font-bold tracking-wide disabled:opacity-60" }, "Normal Practice"),
            React.createElement("button", { onClick: () => { setRaceDurationDraft('120'); setView('raceDuration'); }, disabled: raceSourcePoolIds.length === 0, style: { borderColor: COLORS.red, color: COLORS.red, ...fontStyle }, className: "w-full rounded-xl border-2 py-2.5 font-bold tracking-wide disabled:opacity-50" }, "Race")));
        // =====================================================================
        // SECTION: RACE DURATION SCREEN  (search: SECTION: RACE DURATION)
        // Time entry before a race starts.
        // =====================================================================
    }
    else if (view === 'raceDuration' && selectedDeck) {
        const seconds = Math.max(1, parseInt(raceDurationDraft, 10) || 0);
        content = (React.createElement("div", { className: "flex flex-col gap-4" },
            React.createElement(BackHeader, { colors: COLORS, onBack: () => setView('raceModeChoice'), title: "Race Setup" }),
            React.createElement("div", { className: "rounded-xl border-2 p-4", style: { borderColor: COLORS.rule, backgroundColor: COLORS.card } },
                React.createElement("label", { style: { ...fontStyle, color: COLORS.ink }, className: "text-sm font-bold block mb-2" }, "Race duration (seconds)"),
                React.createElement("input", { type: "number", min: 1, value: raceDurationDraft, onChange: (e) => setRaceDurationDraft(e.target.value), style: { ...monoStyle, borderColor: COLORS.rule, backgroundColor: COLORS.card, color: COLORS.ink }, className: "w-full rounded-lg border-2 px-3 py-2 text-center text-lg focus:outline-none focus:ring-2" })),
            React.createElement("button", { onClick: () => startRace(raceSourcePoolIds, raceModePracticeOrder, seconds), disabled: !(seconds > 0) || raceSourcePoolIds.length === 0, style: { backgroundColor: COLORS.red, ...fontStyle }, className: "w-full rounded-xl py-3 text-white font-bold tracking-wide disabled:opacity-60" }, "Start Race")));
        // =====================================================================
        ...
        // reorder sections, move their <div> blocks. Practice Order now lives
        // per-deck in Deck > Practice (see SECTION: DECK VIEW), not here.
        // =====================================================================
    }
        ...
        // auto-advancing — see FUNCTION: ANSWERING AN MCQ.
        // =====================================================================
    }
        ...
        if (phase === 'question' && currentCard) {
            ...
            content = (React.createElement(SessionSummaryScreen, { colors: COLORS, cards: cards, sessionPoolIds: sessionPoolIds, sessionResults: sessionResults, sessionAffectsProgress: sessionAffectsProgress, onDone: exitPractice }));
        }
        // =====================================================================
        // SECTION: RACE SCREEN  (search: SECTION: RACE SCREEN)
        // The live race: timer bar, End Race button, question, A/B/C/D options.
        // One pass only — no hint, no hold-for-solution (solution is summary-only
        // per spec). When raceRunning goes false (timer hit 0 or End Race tapped
        // or the last card was answered), moves straight to the race summary.
        // =====================================================================
    }
    else if (view === 'race') {
        if (raceRunning) {
            const raceCard = raceOrder.length ? cards.find((c) => c.id === raceOrder[racePos]) : null;
            const progress = raceTotalSeconds ? ((raceTotalSeconds - raceSecondsLeft) / raceTotalSeconds) * 100 : 0;
            content = raceCard ? (React.createElement("div", { className: "flex flex-col gap-4" },
                React.createElement("div", { className: "flex justify-between items-center" },
                    React.createElement("span", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs tracking-widest" },
                        "Q ",
                        racePos + 1,
                        "/",
                        raceOrder.length),
                    React.createElement("span", { style: { ...monoStyle, color: raceSecondsLeft <= 10 ? COLORS.red : COLORS.ink }, className: "text-lg font-bold" }, `${raceSecondsLeft}s`),
                    React.createElement("button", { onClick: endRaceNow, style: { borderColor: COLORS.red, color: COLORS.red, ...fontStyle }, className: "rounded-lg border-2 px-3 py-1 text-xs font-bold" }, "End Race")),
                React.createElement("div", { className: "w-full h-1 rounded-full", style: { backgroundColor: COLORS.rule } },
                    React.createElement("div", { className: "h-1 rounded-full transition-all duration-300", style: { width: `${progress}%`, backgroundColor: COLORS.red } })),
                React.createElement("div", { className: "rounded-2xl border-2 px-5 py-6", style: { borderColor: COLORS.ink, backgroundColor: COLORS.card } },
                    React.createElement(MathText, { text: raceCard.question, style: { ...fontStyle, color: COLORS.ink }, className: "text-base leading-relaxed font-medium block" })),
                React.createElement("div", { className: "flex flex-col gap-3" }, raceOptions.map((opt, i) => {
                    const isWrong = raceFlashWrongIdx === i;
                    const isRight = raceFlashCorrectIdx === i;
                    return (React.createElement("button", { key: i, onClick: () => handleRaceOptionClick(i), disabled: raceFlashCorrectIdx !== null || raceFlashWrongIdx !== null, className: `w-full text-left rounded-xl border-2 px-4 py-3 flex items-center gap-3 transition-colors duration-150 focus:outline-none focus:ring-2 ${isWrong ? 'animate-shake' : ''}`, style: {
                            borderColor: isWrong ? COLORS.red : isRight ? COLORS.green : COLORS.ink,
                            backgroundColor: isWrong ? COLORS.flashRedBg : isRight ? COLORS.flashGreenBg : COLORS.card,
                        } },
                        React.createElement("span", { style: {
                ...
                : (React.createElement("div", { className: "py-16 text-center", style: { ...monoStyle, color: COLORS.inkFaint } }, "loading race..."));
        }
        else {
            content = (React.createElement(RaceSummaryScreen, { colors: COLORS, cards: cards, raceOrder: raceOrder, raceResults: raceResults, onDone: exitRace }));
        }
        // =====================================================================
        // (end SECTION: RACE SCREEN)
        // =====================================================================
    }
    return (React.createElement("div", { style: { backgroundColor: COLORS.rule }, className: "w-full min-h-screen flex justify-center md:items-start md:py-10" },
        ...
        .note-markdown th, .note-markdown td { white-space: nowrap !important; }
...
function NoteBody({ text, className, style }) {
    ...
    return React.createElement("div", { className: `note-markdown ${className || ''}`, style: style, dangerouslySetInnerHTML: { __html: html } });
}
...
// A card missing from sessionResults (shouldn't normally happen — every pool card gets
// answered correctly at least once before 'allClear' is reachable) falls back to "Good"
// with a blank time rather than crashing the summary.
//
...
// sessions actually reschedule anything; Practice All · MCQ and Practice by Difficulty
// are read-only, so the banner just states the card count for those.
//
...
// AN MCQ). Tier colors (tiles + per-card pills): Strong=green, Good=COLORS.incorrect,
// Weak=mustard, Incorrect=red — this mapping is shared with Practice by Difficulty's
// tiers array (SECTION: DIFFICULTY TIERS); the two are kept in sync deliberately, so
// edit both together if this ever changes.
//
...
function SessionSummaryScreen({ colors: COLORS, cards, sessionPoolIds, sessionResults, sessionAffectsProgress, onDone }) {
    ...
    const items = sessionPoolIds.map((id, index) => {
        ...
        return {
            ...
            outcome: result && result.outcome ? result.outcome : 'good',
        };
    });
    const counts = { strong: 0, good: 0, weak: 0, incorrect: 0 };
    ...
    return (React.createElement("div", { className: "flex flex-col gap-4 -mb-4" },
        ...
        React.createElement("div", { className: "sticky top-0 z-10 grid grid-cols-2 gap-2 py-2", style: { backgroundColor: COLORS.paper } }, tiers.map((t) => {
            ...
            return (React.createElement("button", { key: t.key, onClick: () => setFilterTier((prev) => (prev === t.key ? null : t.key)), className: "rounded-xl border-2 px-3 py-2 flex items-center justify-between transition-opacity min-w-0", style: { borderColor: t.color, backgroundColor: t.bg, opacity: dimmed ? 0.4 : 1, boxShadow: active ? `0 0 0 2px ${t.color}` : 'none' } },
        ...
        React.createElement("div", { className: "flex flex-col gap-2" }, visibleItems.map((it) => {
            ...
            return (React.createElement("div", { key: it.id, className: "rounded-xl border-2 px-4 py-3 min-w-0 overflow-x-auto", style: { borderColor: COLORS.rule, backgroundColor: COLORS.card } },
                React.createElement("div", { className: "flex items-start justify-between gap-2" },
                    React.createElement("div", { style: { fontFamily: "'Roboto Slab', serif", color: COLORS.ink }, className: "text-sm font-medium flex-1 min-w-0" },
...
// ===== SECTION: RACE SUMMARY SCREEN (search: SECTION: RACE SUMMARY) =====
// Shown once a race ends (timer hit zero, End Race was tapped, or every card in the
// pool got answered). Same tile/list layout as SessionSummaryScreen, but:
//   - Tiers are renamed for the race framing: Strong->Winners, Good->Potential Winners,
//     Weak->Losers, Incorrect->The Dead — plus a 5th tier, Never Fought, for any card
//     the race never reached (timer ran out or End Race was tapped early).
//   - One pass only: raceResults has at most one entry per card (set the instant it's
//     answered), no retries, so there's no "time to correct" column — just the tier.
//   - Solution is always shown inline when present (not a reveal toggle) — race-mode
//     spec: solution/explanation is summary-only, never shown during the race itself.
//   - Never touches sr/difficulty/backup — this screen only reads raceOrder/raceResults,
//     both of which are discarded (reset next time a race starts) rather than persisted.
function RaceSummaryScreen({ colors: COLORS, cards, raceOrder, raceResults, onDone }) {
    const [filterTier, setFilterTier] = useState(null);
    const [doneVisible, setDoneVisible] = useState(true);
    const lastScrollYRef = useRef(0);
    useEffect(() => {
    ...
    const tiers = [
        ...
        { key: 'neverFought', label: 'Never Fought', color: COLORS.inkFaint, bg: COLORS.neutralBg },
    ];
    const items = raceOrder.map((id, index) => {
        const card = cards.find((c) => c.id === id);
        const outcome = raceResults[id] || 'neverFought';
        return {
            id,
            index,
            question: card ? card.question : '(card no longer exists)',
    ...
    const counts = { strong: 0, good: 0, weak: 0, incorrect: 0, neverFought: 0 };
    items.forEach((it) => { counts[it.outcome] = (counts[it.outcome] || 0) + 1; });
    const visibleItems = filterTier ? items.filter((it) => it.outcome === filterTier) : items;
    return (React.createElement("div", { className: "flex flex-col gap-4 -mb-4" },
        React.createElement("div", { className: "flex flex-col items-center gap-1 pt-2" },
            React.createElement("div", { className: "stamp-el rounded-lg px-6 py-4 text-center", style: { border: `4px double ${COLORS.red}`, backgroundColor: COLORS.redBg, transform: 'rotate(-4deg)' } },
                React.createElement("div", { style: { fontFamily: "'Roboto Slab', serif", color: COLORS.red }, className: "text-xl font-bold tracking-wide" }, "RACE OVER"),
                React.createElement("div", { style: { fontFamily: "'Space Mono', monospace", color: COLORS.red }, className: "text-xs mt-1" }, `${items.length} card${items.length !== 1 ? 's' : ''} \u00B7 no scheduling impact`))),
        React.createElement("div", { className: "sticky top-0 z-10 grid grid-cols-2 gap-2 py-2", style: { backgroundColor: COLORS.paper } }, tiers.map((t) => {
            const active = filterTier === t.key;
            const dimmed = !!filterTier && !active;
            return (React.createElement("button", { key: t.key, onClick: () => setFilterTier((prev) => (prev === t.key ? null : t.key)), className: `rounded-xl border-2 px-3 py-2 flex items-center justify-between transition-opacity min-w-0${t.key === 'neverFought' ? ' col-span-2' : ''}`, style: { borderColor: t.color, backgroundColor: t.bg, opacity: dimmed ? 0.4 : 1, boxShadow: active ? `0 0 0 2px ${t.color}` : 'none' } },
                React.createElement("span", { style: { fontFamily: "'Roboto Slab', serif", color: t.color }, className: "text-xs font-bold" }, t.label),
                React.createElement("span", { style: { fontFamily: "'Space Mono', monospace", color: t.color }, className: "text-sm font-bold" }, counts[t.key])));
        })),
        React.createElement("div", { style: { fontFamily: "'Space Mono', monospace", color: COLORS.inkFaint }, className: "text-xs uppercase tracking-widest" }, filterTier ? tiers.find((t) => t.key === filterTier).label : "All cards"),
        React.createElement("div", { className: "flex flex-col gap-2" }, visibleItems.map((it) => {
            const tier = tiers.find((t) => t.key === it.outcome) || tiers[4];
            return (React.createElement("div", { key: it.id, className: "rounded-xl border-2 px-4 py-3 min-w-0 overflow-x-auto", style: { borderColor: COLORS.rule, backgroundColor: COLORS.card } },
                React.createElement("div", { style: { fontFamily: "'Roboto Slab', serif", color: COLORS.ink }, className: "text-sm font-medium" },
                    React.createElement("span", { style: { fontFamily: "'Space Mono', monospace", color: COLORS.ink }, className: "mr-1" },
                        it.index + 1,
                        "."),
                    React.createElement(MathText, { text: it.question })),
                ...
                React.createElement("div", { className: "flex items-center gap-1.5 flex-wrap mt-1.5" },
                    React.createElement("span", { style: { fontFamily: "'Roboto Slab', serif", color: tier.color, backgroundColor: tier.bg }, className: "inline-block text-[10px] font-bold px-2 py-0.5 rounded-full" }, tier.label)),
                it.solution ? (React.createElement("div", { className: "mt-2 rounded-lg border-2 px-3 py-2 text-xs", style: { borderColor: COLORS.solution, backgroundColor: COLORS.solutionBg, color: COLORS.solutionText } },
                    React.createElement("div", { style: { fontFamily: "'Space Mono', monospace" }, className: "text-[10px] uppercase tracking-widest mb-1 opacity-80" }, "Solution"),
                    React.createElement(MathText, { text: it.solution }))) : null));
        })),
        React.createElement("div", { className: "h-24", "aria-hidden": "true" }),
        React.createElement("div", { className: "fixed inset-x-0 bottom-0 z-20 flex justify-center pointer-events-none" },
            React.createElement("div", { className: "done-bar w-full max-w-sm md:max-w-lg pointer-events-auto", style: {
                ...
                React.createElement("div", { className: "w-full max-w-sm mx-auto px-5 pt-3", style: { paddingBottom: 'calc(0.75rem + env(safe-area-inset-bottom, 0px))' } },
                    React.createElement("button", { onClick: onDone, style: { backgroundColor: COLORS.red, fontFamily: "'Roboto Slab', serif" }, className: "w-full rounded-xl py-3 text-white font-bold tracking-wide" }, "Done"))))));
}
