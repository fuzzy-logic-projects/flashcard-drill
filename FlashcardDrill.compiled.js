...
// again to clear); the tier tile row is the only sticky element (own background,
// pinned under the header while the list scrolls beneath it) — the ALL CLEAR banner
// scrolls away normally so the card list gets full page space. Each card row can
// reveal its hint and/or solution (if it has one) independently, in place, via small
// toggle buttons — useful for a card that graduated on the first try and therefore
// never paused on the in-practice solution screen (see FUNCTION: ANSWERING AN MCQ).
