...
export default function FlashcardDrillApp() {
    ...
    if (!loaded) {
        ...
        const statTile = (label, value, color, caption) => (React.createElement("div", { key: label, className: "rounded-2xl border-2 px-5 py-4 shadow-sm", style: { borderColor: COLORS.rule, backgroundColor: COLORS.card } },
            React.createElement("div", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs tracking-widest uppercase" }, label),
            React.createElement("div", { style: { ...fontStyle, color }, className: "text-3xl font-bold mt-1" }, value),
            React.createElement("div", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-xs mt-1" }, caption)));
        content = (React.createElement("div", { className: "flex flex-col gap-3 lg:gap-6" },
            React.createElement("div", { className: "hidden lg:flex lg:items-end lg:justify-between" },
                React.createElement("div", null,
                    React.createElement("h1", { style: { ...fontStyle, color: COLORS.ink }, className: "text-3xl font-bold" }, "Your Decks"),
                    React.createElement("p", { style: { ...monoStyle, color: COLORS.inkFaint }, className: "text-sm mt-1" },
                        totalDue > 0 ? `${totalDue} card${totalDue !== 1 ? 's' : ''} due across ${decks.length} deck${decks.length !== 1 ? 's' : ''}` : `${decks.length} deck${decks.length !== 1 ? 's' : ''} \u00B7 all caught up`))),
            React.createElement("div", { className: "hidden lg:grid lg:grid-cols-3 lg:gap-4" },
                statTile('Decks', decks.length, COLORS.ink, categories.length ? `across ${categories.length} categor${categories.length !== 1 ? 'ies' : 'y'}` : 'no categories yet'),
                statTile('Cards Due Today', totalDue, totalDue > 0 ? COLORS.mustard : COLORS.green, totalDue > 0 ? 'ready to review' : 'all caught up'),
                statTile('Notes Due Today', totalNotesDue, totalNotesDue > 0 ? COLORS.mustard : COLORS.green, totalNotesDue > 0 ? 'ready to review' : 'all caught up')),
            ...
            React.createElement("button", { onClick: () => {
                ...
                }, disabled: totalDue === 0, style: { backgroundColor: totalDue > 0 ? COLORS.accent : COLORS.disabledBg, ...fontStyle }, className: "w-full rounded-xl py-3 text-white font-bold tracking-wide focus:outline-none focus:ring-2 focus:ring-offset-2 lg:py-4 lg:text-base lg:rounded-2xl" },
                "Review All Due (",
                totalDue,
                ")"),
            React.createElement("div", { className: "flex gap-2 lg:gap-3" },
                React.createElement("button", { onClick: () => setView('newDeck'), style: { borderColor: COLORS.ink, color: COLORS.ink, ...fontStyle }, className: "flex-1 rounded-xl border-2 py-2.5 font-bold flex items-center justify-center gap-1 text-sm focus:outline-none focus:ring-2 lg:py-3 lg:rounded-2xl" },
            ...
            decks.length === 0 ? (React.createElement("p", { style: { ...fontStyle, color: COLORS.inkFaint }, className: "text-sm px-1 mt-2" }, "No decks yet. Create one to start building your first set of questions.")) : searchedDecks.length === 0 ? (React.createElement("p", { style: { ...fontStyle, color: COLORS.inkFaint }, className: "text-sm px-1 mt-2" },
                "No decks match \"",
                searchQuery,
                "\".")) : (React.createElement("div", { className: "flex flex-col gap-4 mt-2 lg:gap-6" }, categoryKeys.map((cat) => (React.createElement("div", { key: cat, className: "flex flex-col gap-2" },
                ...
                React.createElement("div", { className: "grid grid-cols-1 gap-2 lg:grid-cols-2 lg:gap-3 xl:grid-cols-3" }, grouped[cat].map((deck) => {
                    ...
                    return (React.createElement("button", { key: deck.id, onClick: () => {
                            setSelectedDeckId(deck.id);
                            setView('deckChoice');
                        }, className: "w-full text-left rounded-xl border-2 px-4 py-3 flex items-center justify-between gap-2 lg:flex-col lg:items-start lg:justify-start lg:gap-3 lg:p-5 lg:rounded-2xl lg:h-full lg:transition-transform lg:duration-150 lg:hover:-translate-y-0.5 lg:hover:shadow-lg shadow-sm", style: { borderColor: COLORS.ink, backgroundColor: COLORS.card } },
                        React.createElement("div", null,
